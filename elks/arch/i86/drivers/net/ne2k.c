...
static size_t ne2k_read(struct inode *inode, struct file *filp, char *data,
			size_t len)
{
	size_t res;
	...
	while (1) {
		...
		if ((nhdr[0] & ~0x7f21) ||
		    (nhdr[0] ==
		     0)) { //EXPERIMENTAL: Upper byte = block #, max 7f
			...
			printk("$%04x.%02x$", ne2k_getpage(),
			       ne2k_next_pk & 0xff);
			if (verbose)
				printk(EMSG_DMGPKT, dev_name, nhdr[0], nhdr[1]);
...
static size_t ne2k_write(struct inode *inode, struct file *file, char *data,
			 size_t len)
{
	size_t res;
...
static void ne2k_int(int irq, struct pt_regs *regs)
{
	...
	while (1) {
		...
		if (stat & NE2K_STAT_RX) {
			...
			wake_up(&rxwait);
			outb(NE2K_STAT_RX,
			     net_port + EN0_ISR); // Clear intr bit
		}
		...
		/* shortcut for speed */
		if (!(stat & ~(NE2K_STAT_TX | NE2K_STAT_RX | NE2K_STAT_OF)))
			continue;
...
static int ne2k_ioctl(struct inode *inode, struct file *file, unsigned int cmd,
		      unsigned int arg)
{
	int err = 0;
	...
	case IOCTL_ETH_ADDR_SET:
		if ((err = verified_memcpy_fromfs(mac_addr, (byte_t *)arg,
						  6)) != 0)
			break;
...
struct file_operations ne2k_fops = { NULL, /* lseek */
				     ne2k_read,	  ne2k_write,
				     NULL, /* readdir */
				     ne2k_select, ne2k_ioctl,
				     ne2k_open,	  ne2k_release };
...
void INITPROC ne2k_drv_init(void)
{
	...
	while (1) {
		...
		if (j && (j != k)) {
			...
			for (i = 0; i < 16; i++)
				cprom[i] = (char)prom[i] & 0xff;
			ne2k_flags = 0;
		}
		if (j == k && !memcmp(cprom, mac_addr, 5))
		...
		if (!(ne2k_flags & ETHF_8BIT_BUS))
			netif_stat.oflow_keep =
				3; // Experimental: use 3 if 16k buffer
