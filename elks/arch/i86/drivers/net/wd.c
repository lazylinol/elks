...
static int INITPROC wd_probe(void)
{
	...
	if (inb(net_port + 8) == 0xff /* Extra check to avoid soundcard. */
	    || inb(net_port + 9) == 0xff || (tmp & 0xff) != 0xFF)
		return -ENODEV;
	...
	if (((inb(net_port + 1) & 0x01) == 0x01) /* A 16 bit card */
		...
		stop_page =
			WD_STOP_PG16; // FIXME: calculate from config parameter
				      // since some interfaces have large (32k)
				      // buffer ram
		netif_stat.oflow_keep = 3;
	} else {
		model_name[4] =
			'0'; // FIXME: Incorrect if 16bit i/f in 8bit slot */
	...
	if (net_flags & ETHF_8BIT_BUS)
		is_8bit = 1;
	if (net_flags & ETHF_16BIT_BUS)
		is_8bit = 0;  /* overrides 8bit */
...
static void wd_init_8390(int strategy)
{
	...
	outb(E8390_NODMA | E8390_PAGE0 | E8390_STOP, WD_8390_PORT + E8390_CMD);
	outb(0x49 - is_8bit,
	     WD_8390_PORT + EN0_DCFG); /* 0x48 vs 0x49: 8 vs 16 bit */
...
static size_t wd_pack_get(char *data, size_t len)
{
	...
	do {
		...
		if (this_frame !=
		    current_rx_page) /* Very useful for debugging ! */
		...
		if ((rxhdr->status & 0x0fU) != ENRSR_RXOK) {
			...
			if (verbose)
				printk(EMSG_BGSPKT, dev_name, rxhdr->status,
				       rxhdr->next, rxhdr->count);
			break;
		}
		res = rxhdr->count - sizeof(e8390_pkt_hdr);
		if (res > len)
			res = len;
		if (current_rx_page > this_frame ||
		    current_rx_page == WD_FIRST_RX_PG) {
			/* no wrap around */
			...
			size_t len1 = ((stop_page - this_frame) << 8) -
				      sizeof(e8390_pkt_hdr);
			fmemcpy(data, current->t_regs.ds,
				(char *)hdr_start + sizeof(e8390_pkt_hdr),
				net_ram, len1, is_8bit);
			fmemcpy(data + len1, current->t_regs.ds,
				(char *)(WD_FIRST_RX_PG << 8), net_ram,
				res - len1, is_8bit);
		}
	} while (0);

	if (update) { /* don't update if we ran overflow recovery */
		this_frame = (current_rx_page == WD_FIRST_RX_PG) ?
					   stop_page - 1 :
					   current_rx_page - 1;
...
static size_t wd_read(struct inode *inode, struct file *filp, char *data,
		      size_t len)
{
	size_t res = 0;
	...
	do {
		...
		res = wd_pack_get(data,
				  len); /* returns packet data size read */
	} while (0);
...
static size_t wd_write(struct inode *inode, struct file *file, char *data,
		       size_t len)
{
	int res;
...
static word_t wd_tx_stat(void)
{
	return (inb(WD_8390_PORT + E8390_CMD) & E8390_TRANS) ? 0U : WD_STAT_TX;
}
...
static int wd_ioctl(struct inode *inode, struct file *file, unsigned int cmd,
		    unsigned int arg)
{
	int err = 0;
	...
	case IOCTL_ETH_ADDR_GET:
		err = verified_memcpy_tofs((char *)arg, &netif_stat.mac_addr,
					   6U);
		break;
...
struct file_operations wd_fops = { NULL,		      /* lseek */
				   wd_read,   wd_write, NULL, /* readdir */
				   wd_select, wd_ioctl, wd_open, wd_release };
...
void INITPROC wd_drv_init(void)
{
	...
	u = wd_probe();
	printk("eth: %s at %x, irq %d, ram %04x", dev_name, net_port, net_irq,
	       net_ram);
	if (u) {
		printk(" not found\n");
	} else {
...
static void fmemcpy(void *dst_off, seg_t dst_seg, void *src_off, seg_t src_seg,
