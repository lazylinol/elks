...
size_t null_write(struct inode *inode, struct file *filp, char *data,
		  size_t len)
{
...
size_t port_write(struct inode *inode, struct file *filp, char *data,
		  size_t len)
{
	size_t i;
...
size_t full_write(struct inode *inode, struct file *filp, char *data,
		  size_t len)
{
 ...
 * /dev/kmem code
 */
size_t kmem_read(struct inode *inode, register struct file *filp, char *data,
		 size_t len)
{
	...
	fmemcpyb((byte_t *)data, current->t_regs.ds, (byte_t *)soff, sseg,
		 (word_t)len);
	filp->f_pos += len;
	return (size_t)len;
}

size_t kmem_write(struct inode *inode, register struct file *filp, char *data,
		  size_t len)
{
	...
	fmemcpyb((byte_t *)doff, dseg, (byte_t *)data, current->t_regs.ds,
		 (word_t)len);
	filp->f_pos += len;
	return len;
}
