...
static struct serial_info ports[NR_SERIAL] = { { COM1_PORT, COM1_IRQ, 0,
						 DEFAULT_LCR, 0, 0, NULL } };
...
static int rs_write(struct tty *tty)
{
	...
	while (tty->outq.len > 0) {
		unsigned long timeout =
			jiffies +
			4 * HZ / 100; /* 40ms, 300 baud needs 33.3ms */
		/* Wait until transmitter hold buffer empty */
		while (!(inb(port->io + UART_LSR) & UART_LSR_THRE)) {
			if (time_after(
				    jiffies,
				    timeout)) /* waits 40ms max, jiffies updated by hw timer */
				break;
		}
...
static void rs_release(struct tty *tty)
{
	...
	if (--tty->usecount == 0) {
		outb(inb(port->io + UART_IER) & 0xF8,
		     port->io + UART_IER); /* Disable all interrupts */
		tty_freeq(tty);
	}
}
...
void rs_conout(dev_t dev, int c)
{
	...
	unsigned long timeout =
		jiffies + 4 * HZ / 100; /* 40ms, 300 baud needs 33.3ms */
...
static void rs_init(struct serial_info *sp)
{
	...
	/* Set the command word */
	outb(UART_LCR_RTS_ON | UART_LCR_RX_EN | UART_LCR_DTR_ON |
		     UART_LCR_TX_EN,
	     sp->io + UART_LCR);
}
