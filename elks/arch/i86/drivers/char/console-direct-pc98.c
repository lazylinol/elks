...
{
	static unsigned char grb[8] = { 0x00, 0x20, 0x80, 0xA0,
					0x40, 0x60, 0xC0, 0xE0 };
...
static void ScrollUp(register Console *C, int y)
{
	...
	if ((unsigned int)y < MaxRow) {
		fmemcpyb(vp, AttributeSeg, vp + Width, AttributeSeg,
			 (MaxRow - y) * (Width << 1));
		fmemcpyb(vp, C->vseg, vp + Width, C->vseg,
			 (MaxRow - y) * (Width << 1));
	}
...
struct tty_ops dircon_ops = { Console_open, Console_release, Console_write,
			      NULL,	    Console_ioctl,   Console_conout };
...
void INITPROC console_init(void)
{
	...
	printk("Direct console, %s kbd %ux%u" TERM_TYPE
	       "(du virtual consoles)\n",
