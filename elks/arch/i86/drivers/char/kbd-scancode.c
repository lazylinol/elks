 ...
 * command.
 */
static enum { KS_FREE, KS_SETTING_LED_1, KS_SETTING_LED_2 } kb_cmd_state;
...
static void keyboard_irq(int irq, struct pt_regs *regs)
{
	...
	defined(CONFIG_KEYMAP_FR) || defined(CONFIG_KEYMAP_ES)
		...
		if (keyReleased) {
			...
			ModeState &=
				~mode; /* key up: clear these and other modes*/
		} else
	 ...
	 */
		mode = ((ModeState & (CAPS | ALT | CTRL | RSHIFT)) >> 1) |
		       (ModeState & LSHIFT);
	 ...
	 */
		if (mode == 2) {
			...
			if (!key) /* zero caps table value means use normal/shift table*/
				mode = (ModeState & (RSHIFT | LSHIFT)) ?
						     1 :
						     0; /* normal SHIFT for non-CAPS chars*/
			else
				mode = (ModeState & (RSHIFT | LSHIFT)) ?
						     0 :
						     2; /* reversed SHIFT for CAPS characters*/
		}
	 ...
	 */
		...
		if ((ModeState & NUM) && code >= SCAN_KP7) { /* 10 key keypad */
			if (ModeState &
			    LSHIFT) /* LSHIFT added by controller for arrow keys*/
				mode = 0; /* NORMAL for arrow keys*/
			else
				mode = 1; /* SHIFT-.. for keypad keys*/
		}
...
static void kbd_send_cmd(int data)
{
	...
	for (;;) {
		...
		case KS_SETTING_LED_1:
			...
			outb_p(LED_CODE,
			       KEYBD); /* prepare keyboard to accept LED
					   values */
...
{
	del_timer(
		&kb_cmd_timer); /* required in case set_leds called before expiration*/
	kb_cmd_timer.tl_expires =
		jiffies + (2 * HZ / 100); /* every 2/100 second*/
