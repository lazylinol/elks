...
size_t cgatext_read(struct inode *inode, struct file *f, char *data, size_t len)
{
	...
	if (len) {
		fmemcpyb(data, current->t_regs.ds, (char *)f->f_pos,
			 cgatext_mem_SEG, len);
...
size_t cgatext_write(struct inode *inode, register struct file *f, char *data,
		     size_t len)
{
	len = free_len(f, len);

	if (len) {
		fmemcpyb((char *)f->f_pos, cgatext_mem_SEG, data,
			 current->t_regs.ds, len);
