...
struct termios def_vals = { BRKINT | ICRNL,	     /* c_iflag*/
			    OPOST | ONLCR,	     /* c_oflag*/
			    (tcflag_t)(B9600 | CS8), /* c_cflag*/
			    (tcflag_t)(ISIG | ICANON | ECHO |
				       ECHOE), /* c_lflag*/
			    0,		       /* c_line*/
			    {
				    3,	/* VINTR*/
...
void tty_release(struct inode *inode, struct file *file)
{
	...
	if (current->pid == rtty->pgrp) {
		...
		if ((int)rtty->termios.c_cflag &
		    HUPCL) { /* warning truncated to 16 bits*/
...
again:
		...
		if (icanon) {
			...
			if (vtime &&
			    vmin) /* start timeout after first character*/
				nonblock = 1;
		}
...
int tty_ioctl(struct inode *inode, struct file *file, int cmd, char *arg)
{
	...
	switch (cmd) {
	case TCGETS:
		ret = verified_memcpy_tofs(arg, &tty->termios,
					   sizeof(struct termios));
		break;
	case TCSETS:
	case TCSETSW:
	case TCSETSF:
		ret = verified_memcpy_fromfs(&tty->termios, arg,
					     sizeof(struct termios));
...
static struct file_operations tty_fops = {
	...
	tty_read,   tty_write, NULL,	   tty_select,
	tty_ioctl,  tty_open,  tty_release
};
...
void INITPROC tty_init(void)
{
	...
	do {
		ttyp--;
		ttyp->minor =
			(unsigned short int)(-1L); /* set unsigned to -1 */
