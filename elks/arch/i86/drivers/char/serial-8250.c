...
struct serial_info {
	...
	struct tty *tty;
	int pad1, pad2,
		pad3; // round out to 16 bytes for faster addressing of ports[]
};
...
static struct serial_info ports[NR_SERIAL] = {
	{ (char *)COM1_PORT, COM1_IRQ, 0, DEFAULT_LCR, DEFAULT_MCR, 0, NULL, 0,
	  0, 0 },
	{ (char *)COM2_PORT, COM2_IRQ, 0, DEFAULT_LCR, DEFAULT_MCR, 0, NULL, 0,
	  0, 0 },
	{ (char *)COM3_PORT, COM3_IRQ, 0, DEFAULT_LCR, DEFAULT_MCR, 0, NULL, 0,
	  0, 0 },
	{ (char *)COM4_PORT, COM4_IRQ, 0, DEFAULT_LCR, DEFAULT_MCR, 0, NULL, 0,
	  0, 0 },
};
...
static int rs_write(struct tty *tty)
{
	register struct serial_info *port =
		&ports[tty->minor - RS_MINOR_OFFSET];
	int i = 0;
...
void rs_irq(int irq, struct pt_regs *regs)
{
	...
	} while (INB(io + UART_LSR) &
		 UART_LSR_DR); /* while data available (for FIFOs)*/
...
#ifdef CONFIG_FAST_IRQ4
	case 4:
		err = request_irq(port->irq, (irq_handler)_irq_com1,
				  INT_SPECIFIC);
		break;
#endif
#ifdef CONFIG_FAST_IRQ3
	case 3:
		err = request_irq(port->irq, (irq_handler)_irq_com2,
				  INT_SPECIFIC);
		break;
#endif
	default:
...
void INITPROC rs_setbaud(dev_t dev, unsigned long baud)
{
	register struct serial_info *sp = &ports[MINOR(dev) - RS_MINOR_OFFSET];
	register struct tty *tty =
		ttys + NR_CONSOLES + MINOR(dev) - RS_MINOR_OFFSET;
	unsigned int b; /* use smaller 16-bit width to reduce code*/

	if (baud == 115200)
		b = B115200;
	else if ((unsigned)baud == 57600)
		b = B57600;
	else if ((unsigned)baud == 38400)
		b = B38400;
	else if ((unsigned)baud == 19200)
		b = B19200;
	else if ((unsigned)baud == 9600)
		b = B9600;
	else if ((unsigned)baud == 4800)
		b = B4800;
	else if ((unsigned)baud == 2400)
		b = B2400;
	else if ((unsigned)baud == 1200)
		b = B1200;
	else if ((unsigned)baud == 300)
		b = B300;
	else if ((unsigned)baud == 110)
		b = 110;
	else
		return;
...
void INITPROC serial_init(void)
{
	...
	static const char *serial_type[] = {
		"n 8250", " 16450", " 16550", " 16550A", " 16750", " UNKNOWN",
	};
	...
	do {
		if (sp->tty != NULL) {
			printk("ttyS%d at %x, irq %d is a%s\n", ttyno, sp->io,
			       sp->irq, serial_type[sp->flags & SERF_TYPE]);
		}
		sp++;
	} while (++ttyno < NR_SERIAL);
}

struct tty_ops rs_ops = { rs_open, rs_release, rs_write,
			  NULL,	   rs_ioctl,   rs_conout };
