...
static struct serial_info ports[1] = {
	/* Serial 0 */ { PCB_B0CMP, PCB_S0CON, PCB_S0STS, PCB_R0BUF, PCB_T0BUF,
			 UART0_IRQ_RX, UART0_IRQ_TX, 0, &ttys[0] },
...
static const unsigned int baudrate_compares[] = {
	...
	(CONFIG_8018X_FCPU * 1000000UL / (19200UL * 8UL)) -
		1, /* 14 = B19200  */
	(CONFIG_8018X_FCPU * 1000000UL / (38400UL * 8UL)) -
		1, /* 15 = B38400  */
	(CONFIG_8018X_FCPU * 1000000UL / (57600UL * 8UL)) -
		1, /* 16 = B57600  */
	(CONFIG_8018X_FCPU * 1000000UL / (115200UL * 8UL)) -
		1, /* 17 = B115200 */
	(CONFIG_8018X_FCPU * 1000000UL / (230400UL * 8UL)) -
		1, /*  0 = B230400 */
};
...
{
	struct serial_info *port =
		&ports[0]; /* TODO: add support for Serial 1 */
...
struct tty_ops i8018xcon_ops = { sercon_open, sercon_release, sercon_write,
				 NULL,	      sercon_ioctl,   sercon_conout };
