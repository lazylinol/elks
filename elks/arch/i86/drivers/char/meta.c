...
static void do_meta_request(void)
{
	...
	int major =
		udd_major; /* FIXME required until device passed to request_fn*/
	struct ud_driver *driver = get_driver(major);
	struct ud_request *udr;
	char *buff;

	printk("do_meta_request %d %x\n", major,
	       blk_dev[major].current_request);
	if (NULL == driver) {
		end_request(0);
		return;
	}
	while (1) {
		...
		if (req->rq_cmd == WRITE) {
			...
			verified_memcpy_tofs(driver->udd_data, buff,
					     BLOCK_SIZE);
		...
		if (req->rq_cmd == READ) {
#if UNUSED /* FIXME Can't do this, copies to the wrong task */
			verified_memcpy_fromfs(buff, driver->udd_data,
					       BLOCK_SIZE);
			fmemcpyw(buff, kernel_ds, driver->udd_data,
				 driver->udd_task->mm.dseg, 1024 / 2);
#endif
		}
		end_request(1);
...
static int ucd_ioctl(struct inode *inode, struct file *filp, int cmd, char *arg)
{
	printk("ucd_ioctl\n");
	switch (cmd) {
	default:
...
static int meta_ioctl(struct inode *inode, struct file *filp, int cmd,
		      char *arg)
{
	...
	if (minor == 0) {
		...
		if (minor == 0)
			return -ENODEV;
		driver = &drivers[i];
		verified_memcpy_fromfs(driver, arg,
				       sizeof(struct ud_driver_trunc));
		if (driver->udd_type == UDD_CHR_DEV) {
			printk("char ");
			if (register_chrdev(driver->udd_major, "meta",
					    &ucd_fops) < 0) {
				goto reg_err;
			}
			...
			if (register_blkdev(driver->udd_major, "meta",
					    &ubd_fops) < 0) {
				goto reg_err;
			}
		...
		printk("device\n");
		udd_major =
			driver->udd_major; /* FIXME until device passed to request_fn*/
