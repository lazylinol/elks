...
#define NR_SECTS_PC98(p98) \
	((sector_t)END_SECT_PC98(p98) - START_SECT_PC98(p98) + 1)
#define START_SECT_PC98(p98)                                            \
	((sector_t)p98->cyl * last_drive->heads * last_drive->sectors + \
	 p98->head * last_drive->sectors + p98->sector)
#define END_SECT_PC98(p98)                                                  \
	((sector_t)p98->end_cyl * last_drive->heads * last_drive->sectors + \
	 p98->end_head * last_drive->sectors + p98->end_sector)
#endif
				      ...
				      unsigned int minor)
{
	...
	printk("%s%c", hd->major_name,
	       'a' + (unsigned char)(minor >> hd->minor_shift));
				   ...
				   sector_t start, sector_t size)
{
	...
	if (ROOT_DEV == bios_drive_map[minor >> hd->minor_shift]) {
		sector_t boot_start = SETUP_PART_OFFSETLO |
				      (sector_t)SETUP_PART_OFFSETHI << 16;
		if (start == boot_start)
...
static void INITPROC extended_partition(register struct gendisk *hd, kdev_t dev)
{
	...
	while (1) {
		...
		for (i = 0; i < 4; i++, p++) {
			...
			if (i >= 2 && START_SECT(p) + NR_SECTS(p) > this_size &&
...
static int INITPROC msdos_partition(struct gendisk *hd, kdev_t dev,
	    ...
	    *(unsigned short *)(bh->b_data + 0x6) == 0x314C) {
		...
		for (i = 1; i <= 4; minor++, i++, p98++) {
			...
			add_partition(hd, minor,
				      first_sector + START_SECT_PC98(p98),
				      NR_SECTS_PC98(p98));
		}
	}
#endif
