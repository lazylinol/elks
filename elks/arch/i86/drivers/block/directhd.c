...
{
#asm
	push bp mov bp, sp push cx push di push dx mov dx, [bp + 04] mov di,
		[bp + 06] mov cx, [bp + 08] shr cx,
		1 cld
#ifndef CONFIG_PC_XT
			repz insw
#else
			...
			dhd_insw_loop : in ax,
					dx stosw loop dhd_insw_loop
#endif
				pop dx pop di pop cx pop bp
#endasm
}
...
{
#asm
	push bp mov bp, sp push cx push si push dx mov dx, [bp + 04] mov si,
		[bp + 06] mov cx, [bp + 08] shr cx,
		1 cld
#ifndef CONFIG_PC_XT
			repz outsw
			...
			dhd_outsw_loop : /* this should work on an 8088 */
					 lodsw out dx,
					 ax loop dhd_outsw_loop
#endif
				pop dx pop si pop cx pop bp
#endasm
}
	...
	while (1) { /* process HD requests */
	    ...
	    this_pass = count;	/* this is redundant */
			...
			if (count <= (drive_info[drive].sectors - sector + 1))
				...
				this_pass =
					drive_info[drive].sectors - sector + 1;
			...
			if (req->rq_cmd == READ) {
				...
				printk("athd: cyl: %d start: %ld tmp: %d count: %ld di[d].s: %d di[0].s: %d\n",
				       cylinder, start, tmp, count,
				       drive_info[drive].sectors,
				       drive_info[0].sectors);
#endif
				/* read to buffer */
				while (WAITING(port))
					;
