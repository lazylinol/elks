...
#define USE_IMPLIED_SEEK \
	0 /* =1 for QEMU with 360k/AT stretch floppies (not real hw) */
...
/* adjustable timeouts */
#define TIMEOUT_MOTOR_ON \
	(HZ / 2) /* 500 ms wait for floppy motor on before I/O */
#define TIMEOUT_MOTOR_OFF \
	(3 * HZ) /* 3 secs wait for floppy motor off after I/O */
...
static int
	recalibrate; /* seek errors, etc, eventually triggers recalibrate_floppy */
...
#define _MK_LINADDR(seg, offs) \
	((unsigned long)((((unsigned long)(seg)) << 4) + (unsigned)(offs)))
...
static struct floppy_struct minor_types[] = {
	...
	/*1*/
	{ 720, 9, 2, 40, 0, 0x2A, 0x02, 0xDF, 0x50,
	  "360k/PC" }, /* 360kB PC diskettes */
		       /*2*/
	{ 2400, 15, 2, 80, 0, 0x1B, 0x00, 0xDF, 0x54,
	  "1.2M" }, /* 1.2 MB AT-diskettes */
		    /*3*/
	{ 720, 9, 2, 40, 1, 0x2A, 0x02, 0xDF, 0x50,
	  "360k/AT3" }, /* 360kB in 720kB drive */
			/*4*/
	{ 1440, 9, 2, 80, 0, 0x2A, 0x02, 0xDF, 0x50,
	  "720k" }, /* 3.5" 720kB diskette */
		    /*5*/
	{ 720, 9, 2, 40, 1, 0x23, 0x01, 0xDF, 0x50,
	  "360k/AT" }, /* 360kB in 1.2MB drive */
		       /*6*/
	{ 1440, 9, 2, 80, 0, 0x23, 0x01, 0xDF, 0x50,
	  "720k/1.2M" }, /* 720kB in 1.2MB drive */
			 /*7*/
	{ 2880, 18, 2, 80, 0, 0x1B, 0x00, 0xCF, 0x6C,
	  "1.44M" }, /* 3.5" 1.44MB diskette */
		     /*8*/
	{ 5760, 36, 2, 80, 0, 0x1B, 0x43, 0xAF, 0x28,
	  "2.88M" }, /* 3.5" 2.88MB diskette */
 ...
 */
static unsigned char *probe_list[CMOS_MAX] = { p360k, p1200k, p720k, p1440k,
					       p2880k };
...
static char
	tmp_floppy_area[BLOCK_SIZE] WORD_ALIGNED; /* for now FIXME to be removed */
...
static void DFPROC floppy_on(int nr)
{
	...
	DEBUG("flpON");
	*fl_timeout =
		0; /* Reset BIOS motor timeout counter, neccessary on some machines */
...
#pragma GCC diagnostic ignored "-Wshift-count-overflow"
	use_xms = req->rq_seg >>
		  16; /* will be nonzero only if XMS configured & XMS buffer */
	physaddr = (req->rq_seg << 4) + (unsigned int)req->rq_buffer;

	/* FIXME req->rq_nr_sectors will be 0 for TLVC only */
	count = req->rq_nr_sectors ? (unsigned)req->rq_nr_sectors << 9 :
					   BLOCK_SIZE;
	...
	if (read_track) { /* mark buffer-track bad, in case all this fails.. */
		...
		if (floppy->sect & 1 && !head)
			count +=
				512; /* add one if head=0 && sector count is odd */
		dma_addr = _MK_LINADDR(DMASEG, 0);
	} else if (dma_addr >= LAST_DMA_ADDR) {
		dma_addr = _MK_LINADDR(kernel_ds,
				       tmp_floppy_area); /* use bounce buffer */
		if (command == FD_WRITE) {
			xms_fmemcpyw(tmp_floppy_area, kernel_ds,
				     CURRENT->rq_buffer, CURRENT->rq_seg,
				     BLOCK_SIZE / 2);
		}
	}
	...
	outb(FLOPPY_DMA | (command == FD_READ ? DMA_MODE_READ : DMA_MODE_WRITE),
	     DMA_MODE);
...
static void DFPROC configure_fdc_mode(void)
{
	...
	if (fdc_version >= FDC_TYPE_82072 &&
	    (need_configure || USE_IMPLIED_SEEK)) {
		...
		if (implied_seek)
			output_byte(
				0x4A); /* FIFO on, polling on, 10 byte threshold, implied seek */
		else
			output_byte(
...
static void rw_interrupt(void)
{
	...
	case 1: /* error occured during command execution */
		...
		if (ST1 & ST1_WP) {
			...
			if (ST0 & ST0_ECE) {
				...
				printk("unknown error, %x %x %x %x", ST0, ST1,
				       ST2, ST3);
			}
		}
		printk("\n");
	...
	if (read_track) {
		...
		xms_fmemcpyw(CURRENT->rq_buffer, CURRENT->rq_seg, buffer_area,
		   ...
		   && _MK_LINADDR(CURRENT->rq_seg, CURRENT->rq_buffer) >=
			      LAST_DMA_ADDR
#endif
	) {
		...
		xms_fmemcpyw(CURRENT->rq_buffer, CURRENT->rq_seg,
			     tmp_floppy_area, kernel_ds, BLOCK_SIZE / 2);
...
static void DFPROC transfer(void)
{
	...
	if (!seek || (USE_IMPLIED_SEEK && floppy->stretch &&
		      fdc_version >= FDC_TYPE_82072)) {
...
static void recal_interrupt(void)
{
	...
	current_track = NO_TRACK;
	if (result() != 2 ||
	    (ST0 & 0xE0) == 0x60) /* look for SEEK END and ABN TERMINATION*/
		reset = 1;
...
static void floppy_shutdown(void)
{
	DEBUG("[%u]shtdwn0x%x|%x-", (unsigned int)jiffies, current_DOR,
	      running);
	do_floppy = NULL;
...
int check_disk_change(kdev_t dev)
{
	...
	if (!dev && changed_floppies) {
		dev = (changed_floppies & 1) ? MKDEV(MAJOR_NR, 0) :
						     MKDEV(MAJOR_NR, 1);
...
static void DFPROC floppy_ready(void)
{
	...
	if (0
		...
		if (current_type[current_drive]) {
			changed_floppies |=
				1
				<< current_drive; /* this will discard any queued I/O */
			printk("df%d: Disk media change detected, suspending I/O\n",
			       current_drive);
			current_type[current_drive] =
				NULL; /* comment out to keep last media format */
		}
...
repeat:
	...
	if (type > 1 && type < MAX_MINOR)
		...
		if (!floppy) {
			tmp = probing ? base_type[drive][probing - 1] :
					      base_type[drive][0];
			if (!tmp) {
	...
	DEBUG("[%u]redo-%c %d(%s) bl %u;", (unsigned int)jiffies,
	      req->rq_cmd == WRITE ? 'W' : 'R', device, floppy->name,
	      req->rq_sector);
	...
	DEBUG("df%d: %s sector %d CHS %d/%d/%d max %d stretch %d seek %d\n",
	      ...
	      start, track, head, sector, floppy->sect, floppy->stretch,
	      seek_track);
	...
	DEBUG("prep %d|%d,%d|%d-", buffer_track, seek_track, buffer_drive,
	      current_drive);
	...
	if ((((seek_track << 1) + head) == buffer_track) &&
		...
		if (command == FD_READ) { /* requested data is in buffer */
			xms_fmemcpyw(req->rq_buffer, req->rq_seg, buf_ptr,
				     DMASEG, BLOCK_SIZE / 2);
			request_done(1);
			goto repeat;
		} else if (command == FD_WRITE) /* update track buffer */
			xms_fmemcpyw(buf_ptr, DMASEG, req->rq_buffer,
		    ...
		    unsigned int arg)
{
	...
	case HDIO_GETGEO: /* need this one for the sys/makeboot command */
		...
		err = verify_area(VERIFY_WRITE, (void *)loc,
...
static void INITPROC config_types(void)
{
	...
	if (!(base_type[0] = find_base(0, (CMOS_READ(0x10) >> 4) & 0xF)))
		base_type[0] =
			find_base(0, CMOS_360k); /* use 360k if no CMOS */
...
static int floppy_open(struct inode *inode, struct file *filp)
{
	...
	access_count++;
	fd_ref[drive]++;
	inode->i_size = (sector_t)floppy->size
			<< 9; /* NOTE: assumes sector size 512 */
	open_inode = inode;
...
static int DFPROC get_fdc_version(void)
{
	...
	printk("df: direct floppy FDC %s (0x%x), irq %d, dma %d\n", name,
	       reply_buffer[0], FLOPPY_IRQ, FLOPPY_DMA);
