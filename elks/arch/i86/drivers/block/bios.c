...
struct drive_infot fd_types[] = {
	/* AT/PS2 BIOS reported floppy formats*/
	{ 40, 9, 2, 512, 0 },  /* 360k */
	{ 80, 15, 2, 512, 1 }, /* 1.2M */
	{ 80, 9, 2, 512, 2 },  /* 720k */
	{ 80, 18, 2, 512, 3 }, /* 1.4M */
		 ...
		 unsigned seg, unsigned offset)
{
	...
	if (drive != last) {
		bios_disk_reset(
			1); /* fixes QEMU retry when switching drive types #1119 */
		last = drive;
	}
#endif
	BD_AX = cmd | num_sectors;
	BD_CX = (unsigned int)((cylinder << 8) | ((cylinder >> 2) & 0xc0) |
			       sector);
	BD_DX = (head << 8) | drive;
	BD_ES = seg;
	BD_BX = offset;
	...
	debug_bios("BIOSHD(%x): %s CHS %d/%d/%d count %d\n", drive,
		   cmd == BIOSHD_READ ? "read" : "write", cylinder, head,
		   sector, num_sectors);
#ifdef IODELAY
...
int INITPROC bios_gethdinfo(struct drive_infot *drivep)
{
	...
	if (ndrives < 4) {
		for (scsi_id = 0; scsi_id < 7; scsi_id++) {
			...
			if ((call_bios_rvalue == 0) && (BD_DX & 0xff)) {
				...
				device_type =
					BD_BX &
					0xf; /* device_type = 0 for Hard Disk */
				if (device_type == 0)
					bios_drive_map[ndrives++] =
						scsi_id + 0xA0;
			}
			if (ndrives >= 4)
				break;
	...
	for (drive = 0; drive < ndrives; drive++) {
		...
		if (sys_caps & CAP_HD_IDE) { /* Normally PC/AT or higher */
			if (!get_ide_data(
				    drive,
				    drivep)) { /* get CHS from the drive itself */
...
int INITPROC bios_getfdinfo(struct drive_infot *drivep)
{
	...
	if (!call_bios(&bdt)) {
		...
		if (!drives &&
		    ndrives) { /* handle Toshiba T1100 BIOS returning 0 drives */
			for (drive = 0; drive < ndrives; drive++) {
...
#ifdef CONFIG_ARCH_PC98
/* switch device */
void bios_switch_device98(int target, unsigned int device,
...
#ifdef CONFIG_ARCH_PC98
	if (((biosdrive & 0xF0) == 0x80) ||
	    ((biosdrive & 0xF0) == 0xA0)) { /* hard drive*/
		for (minor = 0; minor < 4; minor++) {
			if (biosdrive == bios_drive_map[minor])
				break;
		}
		if (minor >= 4)
			minor = 0;
