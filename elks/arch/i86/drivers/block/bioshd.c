...
static int read_sector(int drive, int cylinder, int sector)
{
	...
	do {
		...
		if (!bios_disk_rw(BIOSHD_READ, 1, drive, cylinder, 0, sector,
				  DMASEG, 0))
...
static void probe_floppy(int target, struct hd_struct *hdp)
{
	...
	if (target >= DRIVE_FD0) { /* the floppy drives */
		...
		if (!read_sector(target, 0, 1)) {
			struct elks_disk_parms __far *parms = _MK_FP(
				DMASEG, drivep->sector_size - 2 -
						sizeof(struct elks_disk_parms));

			/* first check for ELKS parm block */
			if (parms->marker[0] == 'e' &&
			    parms->marker[1] == 'L' &&
			    parms->size >=
				    offsetof(struct elks_disk_parms, size)) {
				...
				if (drivep->cylinders != 0 &&
					...
					printk("fd: found valid ELKS CHS %d,%d,%d disk parameters on /dev/fd%d "
					       "boot sector\n",
					       drivep->cylinders, drivep->heads,
					       drivep->sectors, target);
#endif
					goto got_geom;
				}
			}
			...
			if (
				...
				((boot[3] == 'M' &&
				 ...
				 (boot[3] == 'I' &&
				  boot[4] == 'B')) && /* or 'IBM'*/
				(boot[54] == 'F' &&
				 boot[55] == 'A')) { /* v4.0 fil_sys 'FAT'*/

				/* has valid MSDOS 4.0+ FAT BPB, use it */
				drivep->sectors =
					boot[24];	  /* bpb_sec_per_trk */
				drivep->heads = boot[26]; /* bpb_num_heads */
				unsigned char media =
					boot[21]; /* bpb_media_byte */
				drivep->cylinders =
                        (media == 0xFD)? 40:
		...
		do {
			if (count)
				bios_switch_device98(target, 0x30,
						     drivep); /* 1.44 MB */
			/* skip probing first entry */
			if (count &&
			    read_sector(target, track_probe[count] - 1, 1)) {
				bios_switch_device98(target, 0x90,
						     drivep); /* 1.232 MB */
				break;
			}
			drivep->cylinders = track_probe[count];
		} while (++count <
			 sizeof(track_probe) / sizeof(track_probe[0]));
#else
		do {
			/* skip probing first entry */
			if (count) {
				int res = read_sector(
					target, track_probe[count] - 1, 1);
#if DEBUG_PROBE
				printk("CYL %d %s, ", track_probe[count] - 1,
				       res ? "fail" : "ok");
#endif
				if (res)
					break;
		...
		do {
			...
			if (count &&
			    read_sector(target, 0, sector_probe[count])) {
				bios_switch_device98(target, 0x90,
						     drivep); /* 1.232 MB */
				break;
			}
			drivep->sectors = sector_probe[count];
			 ...
			 sizeof(sector_probe) / sizeof(sector_probe[0]));
#else
		do {
			int res = read_sector(target, 0, sector_probe[count]);
#if DEBUG_PROBE
			printk("SEC %d %s, ", sector_probe[count],
			       res ? "fail" : "ok");
#endif
			if (res) {
				if (count ==
				    0) { /* failed on first sector read, use BIOS parms */
...
void INITPROC bioshd_init(void)
{
...
#ifdef PER_DRIVE_INFO
	{
		...
		for (count = 0; count < NUM_DRIVES; count++, drivep++) {
			if (drivep->heads != 0) {
				char *unit = UNITS;
				__u32 size = ((__u32)drivep->sectors) *
					     5; /* 0.1 kB units */
				if (drivep->sector_size == 1024)
					size <<= 1;
				size *= ((__u32)drivep->cylinders) *
					drivep->heads;
	...
	printk("bioshd: %d floppy drive%s and %d hard drive%s\n", fd_count,
	       fd_count == 1 ? "" : "s", hd_count, hd_count == 1 ? "" : "s");
#else
	...
	if (!register_blkdev(MAJOR_NR, DEVICE_NAME, &bioshd_fops)) {
		...
		if (gendisk_head == NULL) {
			...
			for (ptr = gendisk_head; ptr->next != NULL;
			     ptr = ptr->next)
				continue;
			...
			unsigned int cmd, unsigned int arg)
{
	...
	case HDIO_GETGEO:
		...
		if (!err) {
			...
			put_user_long(hd[MINOR(inode->i_rdev)].start_sect,
				      &loc->start);
		}
	}
	return err;
...
static void get_chst(struct drive_infot *drivep, sector_t *start_sec,
		     unsigned int *c, unsigned int *h, unsigned int *s,
		     unsigned int *t, int fulltrack)
{
	...
	*t = drivep->sectors - *s + 1;
	...
	if (fulltrack) {
		...
		if (*s - 1 <
		    max_sectors) { /* adjust start sector backwards for full track read*/
			*s = 1;
			*t = max_sectors;
			...
			ramdesc_t seg, int cmd, unsigned int count)
{
	...
	if (this_pass > count)
		this_pass = count;
	if (cmd == READ)
		debug_bios("bioshd(%x): read lba %ld count %d\n", drive, start,
			   this_pass);
...
#pragma GCC diagnostic ignored "-Wshift-count-overflow"
		usedmaseg =
			seg >>
			16; /* will be nonzero only if XMS configured and XMS buffer */
		if (!usedmaseg) {
			...
			debug_blk("bioshd: %p:%p = %p count %d wrap %d\n",
				  (unsigned int)seg, buf, physaddr, this_pass,
				  usedmaseg);
		}
		if (usedmaseg) {
			...
			if (cmd == WRITE) /* copy xms buffer down before write*/
				xms_fmemcpyw(0, DMASEG, buf, seg,
					     this_pass * (drivep->sector_size >>
							  1));
			set_cache_invalid();
		} else {
			segment = (seg_t)seg;
		...
		debug_bios("bioshd(%x): cmd %d CHS %d/%d/%d count %d\n", drive,
			   cmd, cylinder, head, sector, this_pass);

		bios_set_ddpt(drivep->sectors);
		error = bios_disk_rw(cmd == WRITE ? BIOSHD_WRITE : BIOSHD_READ,
				     this_pass, drive, cylinder, head, sector,
				     segment, offset);
		if (error) {
			printk("bioshd(%x): cmd %d retry #%d CHS %d/%d/%d count %d\n",
			       drive, cmd, MAX_ERRS - errs + 1, cylinder, head,
			       sector, this_pass);
			bios_disk_reset(drive);
		}
	...
	if (error)
		return 0; /* error message in blk.h */

	if (usedmaseg) {
		if (cmd == READ) /* copy DMASEG up to xms*/
			xms_fmemcpyw(buf, seg, 0, DMASEG,
				     this_pass * (drivep->sector_size >> 1));
		set_cache_invalid();
	}
	return this_pass;
}
...
static void do_readtrack(struct drive_infot *drivep, sector_t start)
{
	...
	do {
		...
		bios_set_ddpt(drivep->sectors);
		error = bios_disk_rw(BIOSHD_READ, num_sectors, drive, cylinder,
				     head, sector, DMASEG, 0);
		if (error) {
			printk("bioshd(%x): track read retry #%d CHS %d/%d/%d count %d\n",
			       drive, errs + 1, cylinder, head, sector,
			       num_sectors);
			bios_disk_reset(drive);
		}
	...
	debug_bios("bioshd(%x): track read lba %ld to %ld count %d\n", drive,
		   cache_startsector, cache_endsector, num_sectors);
}
		       ...
		       ramdesc_t seg)
{
	...
	if (drivep != cache_drive || start < cache_startsector ||
	    start > cache_endsector)
		return 0;
	...
	xms_fmemcpyw(buf, seg, (void *)offset, DMASEG,
		     drivep->sector_size >> 1);
	return 1;
}
...
next_block:
		...
		if (hd[minor].start_sect == -1U ||
		    start + count > hd[minor].nr_sects) {
		...
		while (count > 0) {
			...
			if (drivep - drive_info >= DRIVE_FD0) {
				...
				num_sectors = do_cache_read(drivep, start, buf,
							    req->rq_seg,
							    req->rq_cmd);
			}
			if (!num_sectors)
				...
				num_sectors = do_readwrite(drivep, start, buf,
							   req->rq_seg,
							   req->rq_cmd, count);
		...
		debug_bios("cache: hits %u total %u %lu%%\n", cache_hits,
			   cache_tries, (long)cache_hits * 100L / cache_tries);
