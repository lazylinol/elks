	...
	while (currentp->signal) {
		...
		if (*sd == SIGDISP_DFL) { /* Default */
			if ((mask &	  /* Default Ignore */
			     (SM_SIGCONT | SM_SIGCHLD | SM_SIGWINCH |
			      SM_SIGURG)) ||
			    (currentp->pid == 1 && signr != SIGKILL))
				continue;
			else if (mask & /* Default Stop */
				 (SM_SIGSTOP | SM_SIGTSTP | SM_SIGTTIN |
				  SM_SIGTTOU)) {
				...
				if (mask & /* Default Core */
				    (SM_SIGQUIT | SM_SIGILL | SM_SIGABRT |
				     SM_SIGFPE | SM_SIGSEGV | SM_SIGTRAP))
					dump_core();
#endif
			...
			debug_sig(
				"SIGNAL setup return stack for handler %x:%x\n",
