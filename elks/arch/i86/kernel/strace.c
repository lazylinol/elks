...
static struct sc_info *syscall_info(unsigned int callno)
{
	...
	if (callno < sizeof(elks_table1) / sizeof(struct sc_info)) {
		...
		if (s)
			return s;
	} else if (callno < sizeof(elks_table2) / sizeof(struct sc_info) +
				    START_TABLE2) {
...
#ifdef CHECK_STRACE
static const char *fmtspec[] = { NULL,	 "&0x%X", "0x%X",   "0x%X",
				 "'%c'", "'%c'",  "\"%t\"", "\"%t\"",
				 "%u",	 "%d",	  "&%u",    "&%d",
				 NULL,	 NULL,	  NULL,	    NULL };
...
static void check_kstack(int n)
{
	...
	if (n > currentp->kstack_max) {
		...
		if (currentp->kstack_prevmax != 0) {
			printk("KSTACK(%P) sys_%7s max %3d prevmax %3d sysmax %3d%s",
			       s->s_name, currentp->kstack_max,
			       currentp->kstack_prevmax, max, warning);
			if (n == max)
				printk("*");
			printk("\n");
		}
