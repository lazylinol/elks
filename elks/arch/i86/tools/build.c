...
#define ROOTDEV_ARG 4
	...
	fsz = 0;
	sz = intel_long(ex->a_text) + intel_long(ex->a_data) +
	     intel_long(ex->a_bss);
	...
	fprintf(stderr,
		"System is %d B (%d B code, %d B fartext, %d B data and %u B bss)\n",
		sz, intel_long(ex->a_text), fsz, intel_long(ex->a_data),
	...
	fsz = (ex->a_hdrlen +
	       (intel_long(ex->a_text) + intel_long(ex->a_data)) + 15) /
		      16 +
	      REL_SYSSEG;
	fprintf(stderr, "Text/Data load ending segment is %x, limit %x\n", fsz,
		DEF_SYSSEG);
	if (fsz >
	    DEF_SYSSEG + REL_SYSSEG) { /* start of reloc entry table at boot */
		fprintf(stderr,
			"System too large for DEF_SYSSEG at %x, increase DEF_SYSSEG\n",
			DEF_SYSSEG);
		exit(1);
	}
#endif
