...
int main(int argc, char *argv[])
{
	...
	if (fstat(a_out_fd, &statbuf) < 0) {
		fprintf(stderr,
			"imgconv: Couldn't fstat input file %s: ", argv[1]);
		perror("");
		exit(1);
	}
	...
	if (statbuf.st_size != read(a_out_fd, a_out, statbuf.st_size)) {
		fprintf(stderr,
			"imgconv: Couldn't read %ld bytes of input file %s: ",
			statbuf.st_size, argv[1]);
		perror("");
		exit(1);
	}
	...
	if (NULL != (afl_fd = fopen(aflName, "r"))) {
		...
		for (af = 0; af < MaxAddFiles; af++) {
			...
			if ((af_fd = open(afName, O_RDONLY | O_BINARY)) < 0) {
				fprintf(stderr,
					"imgconv: Couldn't open embedded file %s\n",
					afName);
				exit(1);
			}
			...
			if (fstat(af_fd, &afstatbuf) < 0) {
				fprintf(stderr,
					"imgconv: Couldn't fstat embedded file %s: ",
					afName);
				perror("");
				exit(1);
			}
			...
			if (embeddedFiles[af].length > 0) {
				...
				if ((embeddedFiles[af].data = malloc(
	...
	imgHeader.DataCheckSum = chksum(
		(unsigned char *)(a_out + header->a_text), header->a_data);
	...
	for (i = 0; i < MaxAddFiles; i++) {
		imgHeader.Add[i].offset =
			i < numEmbeddedFiles ? embeddedOffset : 0;
	...
	for (i = 0; i < MaxAddFiles; i++)
		if (embeddedFiles[i].length > 0) {
			fwrite((char *)embeddedFiles[i].data,
			       embeddedFiles[i].length, 1, img_fd);
...
usage: /* Vile, nasty hack: I love it :) */
	...
	fprintf(stderr,
		"option of bcc, e.g. bcc prog.c -o prog.img -Y-p50\n\n");
	fprintf(stderr,
		"options: all <num>'s are in decimal, defaults in brackets\n");
	fprintf(stderr,
		" -C<num>   Specify code version number        (8204)\n");
	fprintf(stderr,
		" -H<num>   Specify number of heap paragraphs  (128)\n");
	fprintf(stderr, " -I<num>   Specify initial value for IP       (0)\n");
	fprintf(stderr,
		" -P<num>   Specify priority                   (128)\n");
	fprintf(stderr, " -S<num>   Specify number of stack paragraphs (64)\n");
	fprintf(stderr,
		" -V        Verbose mode: display code stats   (Don't)\n\n");
	fprintf(stderr,
		"Up to four files may be embedded inside the resultant\n");
	fprintf(stderr,
