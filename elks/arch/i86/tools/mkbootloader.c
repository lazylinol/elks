...
uint32_t aoutsize(char *aout, uint32_t lg)
{
	...
	gr = *(unsigned char *)&(aout[0x04]) + *(uint32_t *)&(aout[0x08]) +
	     *(uint32_t *)&(aout[0x0c]);
	if (gr > lg) {
		gr = lg;
...
int main(int argcnt, char **arg)
{
	...
	if (argcnt < 5) {
		printf("mkbootloader [-r init] [-c start size] target.bin basis_rom [-s][-a] *.bin adr [*.bin adr]\n");
	...
	while (i < argcnt) {
		...
		if (strip) {
			source[nr].lg =
				aoutsize(source[nr].data, source[nr].lg);
		}
		printf("  %s: %xh Bytes %s%s @%04x\n", arg[i],
		       source[nr].lg - source[nr].skipaout,
		       (strip) ? "(strip)" : "", (skip) ? "(- a.out)" : "",
		       source[nr].offs);
		i += 2; // n"achste Quelle
		nr++;
	}
	...
	if ((offs + romgr > 0xfffff) ||
	    ((init >= 0) && (offs + romgr >= 0xffff0))) {
	...
	while (source[nr].data != NULL) {
		...
		while (!l && (source[i].data != NULL)) {
			if (i != nr) {
				if (im(source[nr].offs, source[i].offs,
		...
		if (check.end > romgr) {
			...
			if ((rom[check.start] != 0x55) ||
				...
				if (((check.end - check.start + 1) % 512) != 0)
					printf("Area of checksum use half paragraphs (Start: %x, End: %x => %x)\n",
					       check.start, check.end + 1,
					       check.end - check.start + 1);
				else {
					rom[check.start + 2] =
						(unsigned char)((check.end -
								 check.start +
								 511L) /
								512L);
					printf("  Chksum Nr %d - Length in 512 bytes blocks: %02x   ",
					       check.sum - 1,
					       rom[check.start + 2] & 0xff);
					l = 0;
					rom[check.end] = 0;
					for (i = check.start; i < check.end;
					     i++)
						l += rom[i] & 0xff;
					rom[check.end] = -l;
					printf("[%02hhx @%05x]\n",
					       (unsigned char)(rom[check.end] &
							       0xff),
					       check.end);
					if ((unsigned char)
						    rom[check.start + 2] < 3)
						printf("Some BIOSes have problems with so small devices\n");
					i = 0;
					while (source[i].data != NULL) {
						if (im(check.end,
						       source[i].offs - offs,
