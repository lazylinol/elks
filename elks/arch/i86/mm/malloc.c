...
static segment_s *seg_free_get(segext_t size0, word_t type)
{
	...
	while (n != &_seg_free) {
		...
		if ((seg->flags == SEG_FLAG_FREE) && (size1 >= size00) &&
		    (size1 < best_size)) {
			best_seg = seg;
			best_size = size1;
			incr = size00 - size0;
	...
	if (best_seg) {
		...
		if (incr)
			best_seg = seg_split(best_seg,
					     incr); // split off lower segment
...
{
	segment_s *seg =
		(segment_s *)heap_alloc(sizeof(segment_s), HEAP_TAG_SEG);
	if (seg) {
		seg->base = start;
		seg->size = end - start;
		seg->flags = SEG_FLAG_FREE;
