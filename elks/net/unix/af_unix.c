...
static int unix_bind(struct socket *sock, struct sockaddr *umyaddr,
		     int sockaddr_len)
{
	struct unix_proto_data *upd = sock->data;
	...
	i = do_mknod(upd->sockaddr_un.sun_path,
		     offsetof(struct inode_operations, mknod),
		     S_IFSOCK | S_IRWXUGO, 0);

	if (i == 0)
		i = open_namei(upd->sockaddr_un.sun_path, 0, S_IFSOCK,
			       &upd->inode, NULL);
...
static int unix_connect(struct socket *sock, struct sockaddr *uservaddr,
			int sockaddr_len, int flags)
{
	...
	if (get_user(&(((struct sockaddr_un *)uservaddr)->sun_family)) !=
	    AF_UNIX)
		return -EINVAL;
...
static int unix_getname(struct socket *sock, struct sockaddr *usockaddr,
...
struct proto_ops unix_proto_ops = {
	PF_UNIX,      unix_create,  unix_dup,	     unix_release,
	unix_bind,    unix_connect, unix_socketpair, unix_accept,
	unix_getname, unix_read,    unix_write,	     unix_select,
	NULL,	      unix_listen,  unix_send,	     unix_recv,
	NULL, /* sendto */
	NULL, /* recvfrom */
	NULL, /* shutdown */
	NULL, /* setsockopt */
