...
static struct socket *sock_alloc(void)
{
	...
	*sock = ini_sock;
	...
	sock->wait = (struct wait_queue *)
		inode;	     /* sockets wait on same wq as inodes locks */
...
static size_t sock_read(struct inode *inode, struct file *file, char *ubuf,
			size_t size)
{
	register struct socket *sock;
	int err;
...
static size_t sock_write(struct inode *inode, struct file *file, char *ubuf,
			 size_t size)
{
	register struct socket *sock;
	int err;
...
#if defined(CONFIG_UNIX) || defined(CONFIG_NANO)
int sock_awaitconn(register struct socket *mysock, struct socket *servsock,
		   int flags)
{
...
int sys_accept(int fd, struct sockaddr *upeer_sockaddr, int *upeer_addrlen)
{
	...
	if (upeer_sockaddr)
		newsock->ops->getname(newsock, upeer_sockaddr, upeer_addrlen,
				      1);
...
int sys_socket(int family, int type, int protocol)
{
	...
	ops = find_protocol_family(
		family); /* Initially pops is not an array. */
	if (ops == NULL)
		return -EINVAL;
...
int sys_getsocknam(int fd, struct sockaddr *usockaddr, int *usockaddr_len,
		   int peer)
{
	struct socket *sock;
