...
int inet_process_tcpdev(register char *buf, int len)
{
	...
	debug_net("INET(%P) process_tcpdev sock %x type %d wait %x\n", sock,
		  ((struct tdb_return_data *)buf)->type, sock->wait);

	switch (((struct tdb_return_data *)buf)->type) {
	case TDT_CHG_STATE:
		sock->state = (unsigned char)((struct tdb_return_data *)buf)
				      ->ret_value;
	...
	case TDT_BIND:
		debug_net("INET(%P) retval %d bufin %d\n",
			  ((struct tdb_return_data *)buf)->ret_value,
			  bufin_sem);
			...
			size_t sockaddr_len, int flags)
{
	...
	if (get_user(&(((struct sockaddr_in *)uservaddr)->sin_family)) !=
	    AF_INET)
		return -EINVAL;
...
static int inet_accept(register struct socket *sock, struct socket *newsock,
		       int flags)
{
...
static int inet_read(struct socket *sock, char *ubuf, int size, int nonblock)
{
	...
	while (sock->avail_data == 0) {
		...
		debug_net(
			"INET(%P) read waiting on sock->avail_data sock %x buf_in %d\n",
			sock, bufin_sem);
	...
	if (ret > 0) {
		...
		debug_net("INET(%P) READ %d ask %u avail %u\n", ret, size,
			  sock->avail_data);
...
static struct proto_ops inet_proto_ops = {
	AF_INET,	 inet_create,  inet_dup,
	inet_release,	 inet_bind,    inet_connect,
	not_implemented, /* inet_socketpair */
	inet_accept,	 inet_getname, inet_read,
	inet_write,	 inet_select,  not_implemented, /* inet_ioctl */
	inet_listen,	 inet_send,    inet_recv,
