...
static int nano_bind(struct socket *sock, struct sockaddr *umyaddr,
		     int sockaddr_len)
{
	register struct nano_proto_data *upd = sock->data;
...
static int nano_connect(register struct socket *sock,
			struct sockaddr *uservaddr, int sockaddr_len, int flags)
{
	...
	if (get_user(&(((struct sockaddr_na *)uservaddr)->sun_family)) !=
	    AF_NANO) {
		printk("ADR - {%d}\n", sockna.sun_family);
		return -EINVAL;
	}
	...
	if (sock->conn) {
		...
		NA_DATA(sock)->npd_peerupd =
			NA_DATA(sock->conn); /* ref server */
	}
...
{
	register struct nano_proto_data *upd1 = NA_DATA(sock1),
					*upd2 = NA_DATA(sock2);
...
static int nano_accept(register struct socket *sock, struct socket *newsock,
		       int flags)
{
			...
			int peer)
{
	...
	return move_addr_to_user((char *)&upd->npd_sockaddr_na,
				 upd->npd_sockaddr_len, (char *)usockaddr,
				 usockaddr_len);
}

static int nano_read(register struct socket *sock, char *ubuf, int size,
		     int nonblock)
{
	register struct nano_proto_data *upd;
	int todo, avail;
	...
	while (!(avail = NA_BUF_AVAIL(upd))) {
		if (sock->state != SS_CONNECTED)
			return ((sock->state == SS_DISCONNECTING) ? 0 :
									  -EINVAL);
 ...
 */
	...
	do {
		...
		upd->npd_bp_tail = (upd->npd_bp_tail + cando) &
				   (NA_BUF_SIZE - 1);
		ubuf += cando;
		todo -= cando;
...
static int nano_write(register struct socket *sock, char *ubuf, int size,
		      int nonblock)
{
	register struct nano_proto_data *pupd;
	int todo, space;
 ...
 */
	 ...
	 */
		...
		pupd->npd_bp_head = (pupd->npd_bp_head + cando) &
				    (NA_BUF_SIZE - 1);
...
static int nano_send(struct socket *sock, void *buff, int len, int nonblock,
		     unsigned int flags)
{
	if (flags != 0)
		return -EINVAL;
...
static int nano_recv(struct socket *sock, void *buff, int len, int nonblock,
		     unsigned flags)
{
	if (flags != 0)
		return -EINVAL;
...
struct proto_ops nano_proto_ops = {
	...
	PF_NANO,      nano_create,  nano_dup,	     nano_release,
	nano_bind,    nano_connect, nano_socketpair, nano_accept,
	nano_getname, nano_read,    nano_write,	     nano_select,
	NULL,	      nano_listen,  nano_send,	     nano_recv,
	NULL, /* sendto */
	NULL, /* recvfrom */
	NULL, /* shutdown */
	NULL, /* setsockopt */
