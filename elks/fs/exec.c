...
#define add_overflow(a, b, res) (*(res) = (a) + (b), *(res) < (b))
#define bytes_to_paras(bytes) ((segext_t)(((unsigned long)(bytes) + 15) >> 4))
#else
#define add_overflow __builtin_add_overflow
#define bytes_to_paras(bytes)                                                  \
	__extension__({                                                        \
		segext_t __w;                                                  \
		asm("addw $15, %0; rcrw %0" : "=&r"(__w) : "0"(bytes) : "cc"); \
		__w >> 3;                                                      \
	})
#endif
		    ...
		    size_t tseg)
{
	...
	while (rsize >= sizeof(struct minix_reloc)) {
		...
		word_t val;
		retval = filp->f_op->read(inode, filp, (char *)&reloc,
					  sizeof(reloc));
		...
		case R_SEGWORD:
			...
			case S_TEXT:
				...
				break;
			case S_FTEXT:
				val = seg_code->base + bytes_to_paras(tseg);
				break;
			case S_DATA:
				val = seg_data->base;
				break;
			...
			debug("EXEC: reloc %d,%d: %04x, %x, %x\n", reloc.r_type,
			      reloc.r_symndx, (word_t)reloc.r_vaddr, place_base,
			      val);
...
int sys_execve(const char *filename, char *sptr, size_t slen)
{
	...
	currentp = &task[0];
	do {
		if ((currentp->state <= TASK_STOPPED) &&
	...
	case EXEC_FARTEXT_HDR_SIZE:
		...
		if (add_overflow(min_len, base_data,
				 &min_len)) /* adds stack size*/
			goto error_exec3;
#else
		if (base_data != 0)
			goto error_exec3;
	...
	case 1:
		...
		if (heap >= 0xFFF0) { /* max heap specified*/
			if (len <
			    0xFFF0) /* len could be near overflow from above*/
				len = 0xFFF0;
		} else {
	...
	case 0:
...
#endif
			{
				if (add_overflow(len, INIT_HEAP + INIT_STACK,
						 &len)) {
					retval = -EFBIG;
					goto error_exec3;
				}
		...
		debug("EXEC v0: stack %u heap %u env %u total %u\n", stack,
		      len - min_len - stack - slen, slen, len);
	}
	...
	if (!seg_code) {
		...
		retval = -ENOEXEC;
		if (esuph.esh_compr_tseg &&
		    decompress(0, seg_code->base, (size_t)mh.tseg, bytes, 16) !=
			    (size_t)mh.tseg)
			goto error_exec4;
#endif
#ifdef CONFIG_EXEC_MMODEL
		...
		if (bytes) {
			...
			if (esuph.esh_compr_ftseg &&
			    decompress(0,
				       seg_code->base +
					       bytes_to_paras((size_t)mh.tseg),
				       (size_t)esuph.esh_ftseg, bytes,
				       16) != (size_t)esuph.esh_ftseg)
				goto error_exec4;
#endif
		}
#endif
		...
		seg_get(seg_code);
#ifdef CONFIG_EXEC_MMODEL
		filp->f_pos += esuph.esh_compr_tseg ? esuph.esh_compr_tseg :
							    (size_t)mh.tseg;
		filp->f_pos += esuph.esh_compr_ftseg ? esuph.esh_compr_ftseg :
							     (size_t)esuph.esh_ftseg;
		need_reloc_code = 0;
#else
...
#ifdef CONFIG_EXEC_COMPRESS
	if (esuph.esh_compr_dseg &&
	    decompress(0, seg_data->base, (size_t)mh.dseg, bytes, 16) !=
		    (size_t)mh.dseg)
		goto error_exec5;
#endif
	...
	if (need_reloc_code) {
		...
		retval = relocate(seg_code->base, esuph.msh_trsize, seg_code,
				  seg_data, inode, filp, mh.tseg);
		if (retval != 0)
			goto error_exec5;
		/* Read and apply far text segment relocations */
		retval = relocate(seg_code->base +
					  bytes_to_paras((size_t)mh.tseg),
				  esuph.esh_ftrsize, seg_code, seg_data, inode,
				  filp, mh.tseg);
		if (retval != 0)
			goto error_exec5;
	} else {
	...
	if (currentp->mm.seg_code)
		seg_put(currentp->mm.seg_code);
	if (currentp->mm.seg_data)
		seg_put(currentp->mm.seg_data);
	...
	/* Wipe the BSS */
	fmemsetb((char *)(size_t)mh.dseg + base_data, seg_data->base, 0,
		 (size_t)mh.bseg);
	{
		register int i = 0;
	 ...
	 * gets the right strings. */
		...
		do {
			...
			if ((retval = get_ustack(currentp, i)) != 0)
				put_ustack(currentp, i,
					   (currentp->t_begstack + retval));
			else
				slen++; /* increments for each array traversed */
		} while (slen < 2);
		retval = 0;
	...
	currentp->t_enddata =
		(__pptr)((__u16)mh.dseg + (__u16)mh.bseg + base_data);
