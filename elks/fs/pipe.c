...
int pipe_lseek(struct inode *inode, struct file *file, loff_t offset, int orig)
{
			...
			char *buf, size_t count)
{
	...
	while (PIPE_EMPTY(inode) || PIPE_LOCK(inode)) {
		if (!PIPE_LOCK(inode) && !PIPE_WRITERS(inode))
			return 0;
		if (filp->f_flags & O_NONBLOCK)
			return -EAGAIN;
		if (current->signal)
			return -ERESTARTSYS; // FIXME
		interruptible_sleep_on(&PIPE_WAIT(inode));
	}
	PIPE_LOCK(inode)++;
	if (count > PIPE_LEN(inode))
		count = PIPE_LEN(inode);
			 ...
			 char *buf, size_t count)
{
	...
	while (count > 0) {
		while (((PIPE_SIZE(inode) - PIPE_LEN(inode)) < free) ||
		       PIPE_LOCK(inode)) {
			if (!PIPE_READERS(inode)) {
snd_signal:
		...
		PIPE_LOCK(inode)++;
		while (count > 0 &&
		       (free = (PIPE_SIZE(inode) - PIPE_LEN(inode)))) {
...
#ifdef STRICT_PIPES
static size_t bad_pipe_rw(struct inode *inode, struct file *filp, char *buf,
			  int count)
{
...
struct file_operations rdwr_pipe_fops = {
	pipe_lseek,	pipe_read,	   pipe_write, NULL, /* no readdir */
	NULL,						     /* select */
	NULL,						     /* ioctl */
	pipe_rdwr_open, pipe_rdwr_release,
};
...
static int do_pipe(register int *fd)
{
	...
	if (!(inode = new_inode(NULL, S_IFIFO | S_IRUSR |
					      S_IWUSR))) /* Create inode */
		goto no_inodes;
