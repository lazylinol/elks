			     ...
			     cluster_t new_value)
{
	...
	if (!(bh = msdos_sread(sb,
			       (sector_t)(MSDOS_SB(sb)->fat_start +
					  (first >> SECTOR_BITS_SB(sb))),
			       &data))) {
	...
	{
		next = ((unsigned long *)
				data)[(first & (SECTOR_SIZE_SB(sb) - 1)) >> 2];
		if (next >= 0xffffff8L)
			next = -1;
	}
#ifndef FAT_BITS_32
		...
		next = ((unsigned short *)
				data)[(first & (SECTOR_SIZE_SB(sb) - 1)) >> 1];
		if (next >= 0xfff8)
			next = -1;
	} else { /* fat12*/
		p_first = &((
			unsigned char *)data)[first & (SECTOR_SIZE_SB(sb) - 1)];
		p_last = &(
			(unsigned char *)
				data2)[(first + 1) & (SECTOR_SIZE_SB(sb) - 1)];
		if (this & 1)
	...
	if (new_value != -1) {
		...
		if (fatsz == 32)
			...
			if (bh != bh2)
				debug_fat("fat_access block bh2 dirty %lu\n",
					  buffer_blocknr(bh2));
		...
		for (copy = 1; copy < MSDOS_SB(sb)->fats; copy++) {
			...
			if (data != data2 || bh != bh2) {
				if (!(c_bh2 = msdos_sread(
					      sb,
					      (sector_t)(
						      MSDOS_SB(sb)->fat_start +
						      (first >>
						       SECTOR_BITS_SB(sb)) +
						      MSDOS_SB(sb)->fat_length *
							      copy +
				...
				mark_buffer_dirty(
					c_bh2); //FIXME looks like bug without this
...
void FATPROC cache_init(void)
{
	...
	for (count = 0; count < FAT_CACHE; count++) {
		...
		cache[count].next =
			count == (FAT_CACHE - 1) ? NULL : &cache[count + 1];
	}
	initialized = 1;
}
...
cluster_t FATPROC get_cluster(register struct inode *inode, cluster_t cluster)
{
	...
	if (!(this = inode->u.msdos_i.i_start))
		return 0;
	if (!cluster)
		return this;
	count = 0;
...
sector_t FATPROC msdos_smap(struct inode *inode, sector_t sector)
{
	...
	if ((sb->fat_bits != 32) &&
		...
		if (sector >= sb->dir_entries >> MSDOS_DPS_BITS(inode))
			return 0;
...
#ifndef FAT_BITS_32
			   MSDOS_SB(inode->i_sb)->fat_bits == 12 ? 0xff8UL :
			   MSDOS_SB(inode->i_sb)->fat_bits == 16 ? 0xfff8UL :
#endif
									 0xffffff8UL);
	else {
