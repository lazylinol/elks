...
static int FATPROC msdos_format_name(register const char *name, int len,
				     char *res)
{
	register char *walk;
	unsigned char c;

	if (get_fs_byte(name) == DELETED_FLAG)
		return -EINVAL;
	if (get_fs_byte(name) == '.' &&
				   ...
				   struct buffer_head **bh, ino_t *ino)
{
	...
	*bh = NULL;
	do {
		res = msdos_get_entry_long(dir, &pos, bh, entry_name,
					   &entry_len, &dirpos, ino);
		if (res) {
			if (len == entry_len) {
#ifdef CONFIG_FS_DEV
				/* compare /dev entries case-insensitive for ttyS0 etc*/
				nocase = (dir->i_ino ==
					  MSDOS_SB(dir->i_sb)->dev_ino);
#endif
				if (compare(entry_name, msdos_name, len,
					    nocase))
					return 0;
			}
		}
...
static int FATPROC msdos_create_entry(struct inode *dir, const char *name,
...
int msdos_create(register struct inode *dir, const char *name, int len,
...
int msdos_rmdir(register struct inode *dir, const char *name, int len)
{
	...
	if (inode->u.msdos_i.i_start) { /* may be zero in mkdir */
		...
		while (msdos_get_entry(inode, &pos, &dbh, &dde) != (ino_t)-1)
			if (dde->name[0] &&
			    (unsigned char)dde->name[0] != DELETED_FLAG &&
...
unlink_done:
	unmap_brelse(bh);
	if (inode)
		debug_fat("unlink iput inode %u dirt %d count %d\n",
			  inode->i_ino, inode->i_dirt, inode->i_count);
	if (dir)
		debug_fat("unlink iput dir %u dirt %d count %d\n", dir->i_ino,
			  dir->i_dirt, dir->i_count);
	iput(inode);
	iput(dir);
	return res;
}
