...
static size_t msdos_file_read(struct inode *inode, struct file *filp, char *buf,
			      size_t count);
static size_t msdos_file_write(struct inode *inode, struct file *filp,
...
static size_t msdos_file_read(register struct inode *inode,
			      register struct file *filp, char *buf,
			      size_t count)
{
	char *start;
	...
	if (filp->f_pos >= inode->i_size || count <= 0)
		return 0;
	start = buf;
	while ((left = MIN(inode->i_size - filp->f_pos,
			   count - (buf - start))) != 0) {
		if (!(sector = msdos_smap(inode,
					  filp->f_pos >> SECTOR_BITS(inode))))
			break;
		...
		xms_fmemcpyb(buf, current->t_regs.ds,
			     buffer_data(bh) + offset + secoff, buffer_seg(bh),
			     size);
		buf += size;
		brelse(bh);
	}
...
static size_t msdos_file_write(register struct inode *inode,
			       register struct file *filp, char *buf,
			       size_t count)
{
	sector_t sector;
 ...
 */
	...
	for (start = buf; count; count -= size) {
		while (!(sector = msdos_smap(
				 inode, filp->f_pos >> SECTOR_BITS(inode))))
...
void msdos_truncate(register struct inode *inode)
{
	...
	debug_fat("truncate\n");
	cluster = (cluster_t)SECTOR_SIZE(inode) *
