...
{
	if (inode->i_dirt)
		debug_fat("put_inode %ld count %d dirty %d\n",
			  (unsigned long)inode->i_ino, inode->i_count,
			  inode->i_dirt);
	if (!inode->i_nlink) {
		inode->i_size = 0;
					    ...
					    int silent)
{
	...
	case 512:
		...
		sb->msdos_dps =
			16; /* SECTOR_SIZE / sizeof(struct msdos_dir_entry) */
		sb->msdos_dps_bits = 4; /* log2(msdos_dps) */
		break;
	case 1024:
		sb->sector_bits = 10; /* log2(sector_size) */
		sb->msdos_dps =
			32; /* SECTOR_SIZE / sizeof(struct msdos_dir_entry) */
	...
	sb->data_start = sb->dir_start + (sb->dir_entries >>
					  (SECTOR_BITS_SB(s) - MSDOS_DIR_BITS));
	...
	sb->fat_bits = fat32				       ? 32 :
		       sb->clusters > MSDOS_FAT12_MAX_CLUSTERS ? 16 :
								       12;
	...
	printk("FAT: me=%x,csz=%d,#f=%d,floc=%d,fsz=%d,rloc=%d,#d=%d,dloc=%d,#s=%ld,ts=%ld\n",
	       ...
	       sb->fat_length, sb->dir_start, sb->dir_entries, sb->data_start,
	...
	total_displayed = total_sectors >>
			  (BLOCK_SIZE_BITS - SECTOR_BITS_SB(s));
	...
	free_displayed = free_displayed >>
			 (BLOCK_SIZE_BITS - SECTOR_BITS_SB(s));
#endif
	printk("FAT: total ");
	print_formatted(total_displayed);
...
void msdos_read_inode(register struct inode *inode)
{
...
#endif
		{
			nr = inode->u.msdos_i.i_start =
				MSDOS_SB(inode->i_sb)->root_cluster;
			if (nr) {
				while (nr != -1) {
					inode->i_size +=
						(cluster_t)SECTOR_SIZE(inode) *
						MSDOS_SB(inode->i_sb)
							->cluster_size;
					if (!(nr = fat_access(inode->i_sb, nr,
							      -1L))) {
	...
	map_buffer(bh);
	raw_entry =
		&((struct msdos_dir_entry *)(bh->b_data))[(int)inode->i_ino &
							  (MSDOS_DPB - 1)];
	inode->u.msdos_i.i_start =
		raw_entry->start |
#ifndef FAT_BITS_32
	...
	if (raw_entry->attr & ATTR_DIR) {
		inode->i_mode = MSDOS_MKMODE(raw_entry->attr,
					     0777 & ~current->fs.umask) |
				S_IFDIR;
		inode->i_op = &msdos_dir_inode_operations;
		inode->i_nlink = 3;
		inode->i_size = 0;
		/* read FAT chain to set directory size */
		for (this = inode->u.msdos_i.i_start; this && this != -1;
		     this = fat_access(inode->i_sb, this, -1L))
			inode->i_size += (cluster_t)SECTOR_SIZE(inode) *
					 MSDOS_SB(inode->i_sb)->cluster_size;
	} else {
		inode->i_mode = MSDOS_MKMODE(raw_entry->attr,
					     0755 & ~current->fs.umask) |
				S_IFREG;
...
static struct super_operations msdos_sops = { msdos_read_inode,
					      msdos_write_inode,
					      msdos_put_inode,
					      msdos_put_super,
					      NULL, /* write_super*/
...
struct file_system_type msdos_fs_type = { msdos_read_super, FST_MSDOS };
