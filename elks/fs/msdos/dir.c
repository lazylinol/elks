...
static size_t msdos_dir_read(struct inode *dir, struct file *filp, char *buf,
			     size_t count)
{
	return -EISDIR;
}
...
static int FATPROC unicode_to_ascii(char *ascii, unsigned char *uni)
{
	char *op = ascii;
 ...
 */
int FATPROC
msdos_get_entry_long(struct inode *dir,	      /* scan this directory*/
		     ...
		     ino_t *ino)    /* return entry inode*/
{
	...
	ASYNCIO_REENTRANT unsigned short
		unicodename[26 + 1]; /* Limited to two long entries */

	if ((int)*pos & (sizeof(struct msdos_dir_entry) - 1))
		return -ENOENT;
	is_long = 0;
	*ino = msdos_get_entry(dir, pos, bh, &de);
	...
	while (*ino != (ino_t)-1L) {
		if (de->name[0] == 0) { /* empty  entry and stop reading*/
			break;
		} else if ((unsigned char)de->name[0] ==
			   DELETED_FLAG) { /* empty entry*/
			is_long = 0;
			oldpos = *pos;
		} else if (de->attr == ATTR_EXT) { /* long filename entry*/
			int slot = 0;
			register struct msdos_dir_slot *ds =
				(struct msdos_dir_slot *)de;
			...
			while (slot > 0) {   /* read entries until slot 0*/
				...
				if (slot < 2) {
					int offset =
						slot * 13; /* 13 chars/entry*/
					memcpy(&unicodename[offset],
					       ds->name0_4, 10);
					offset += 5;
					memcpy(&unicodename[offset],
					       ds->name5_10, 12);
				...
				if (slot > 0) {
					*ino = msdos_get_entry(dir, pos, bh,
							       &de);
					if (*ino == (ino_t)-1L) {
						is_long = 0;
						break;
			   ...
			   !(de->attr &
			     ATTR_VOLUME)) { /* short filename entry*/
			int i, i2, last;
			int long_len = 0;
			unsigned char c;
...
#pragma GCC diagnostic ignored "-Waggressive-loop-optimizations"
				...
				unicodename[14] =
					0; /* truncate name to 14 characters*/
				long_len = unicode_to_ascii(
					longname, (unsigned char *)unicodename);
				/* FIXME should handle:
			...
			for (i = last = 0; i < 8; i++) {
				if (!(c = de->name[i]))
					break;
				if (c == 0x05)
					c = 0xE5; /* 05 -> E5 as same as deleted entry*/
				if (c != ' ')
					last = i + 1;
				name[i] = tolower(c);
			}
			 ...
			 filldir_t filldir)
{
	...
	if (dir->i_ino == MSDOS_ROOT_INO) {
		...
		if ((int)filp->f_pos < 2) {
			...
			return filldir(dirbuf, (char *)"..", namlen,
				       filp->f_pos, (ino_t)MSDOS_ROOT_INO);
		}
		if ((int)filp->f_pos ==
		    2) /* reset to 0 after '..' for real directory offset*/
			filp->f_pos = 0;
	}

	res = msdos_get_entry_long(dir, &filp->f_pos, &bh, name, &namelen,
				   &dirpos, &ino);
	if (res > 0)
