...
struct buffer_head *FATPROC msdos_sread_nomap(struct super_block *s,
					      sector_t sector, size_t *offset)
{
	struct buffer_head *bh;

	if (!(bh = bread32(s->s_dev,
			   sector >> (BLOCK_SIZE_BITS - SECTOR_BITS_SB(s)))))
		return NULL;

	//debug_fat("msread sector %ld block %lu\n", sector, buffer_blocknr(bh));
	*offset = ((int)sector & (BLOCK_SIZE_BITS - SECTOR_BITS_SB(s)))
		  << SECTOR_BITS_SB(s);
	return bh;
}

struct buffer_head *FATPROC msdos_sread(struct super_block *s, sector_t sector,
					void **start)
{
	struct buffer_head *bh;
	size_t offset;
...
int FATPROC msdos_add_cluster(register struct inode *inode)
{
	...
	for (count = 0; count < limit; count++) {
		...
		if (fat_access(inode->i_sb, this, -1L) == 0)
			break;
	}
	...
	if (!S_ISDIR(inode->i_mode)) {
		last = inode->i_size ?
				     get_cluster(inode, (inode->i_size - 1) /
							  SECTOR_SIZE(inode) /
							  sb->cluster_size) :
				     0;
	} else {
	...
	for (curr = 0; curr < sb->cluster_size; curr++) {
		...
		if (curr < sb->cluster_size - 1 && !(sector & 1)) {
			if (!(bh = getblk32(inode->i_dev, sector >> 1)))
				...
				debug_blk(
					"msdos_add_cluster1: block %ld uptodate %d\n",
					EBH(bh)->b_blocknr,
					EBH(bh)->b_uptodate);
			...
			if (!(bh = msdos_sread_nomap(inode->i_sb, sector,
						     &offset)))
				printk("FAT: sread fail\n");
			else {
				debug_blk(
					"msdos_add_cluster2: block %ld uptodate %d\n",
					EBH(bh)->b_blocknr,
					EBH(bh)->b_uptodate);
	...
	if (S_ISDIR(inode->i_mode)) {
		...
		inode->i_size +=
			(cluster_t)SECTOR_SIZE(inode) * sb->cluster_size;
		inode->i_dirt = 1;
...
static int day_n[] = { 0,   31,	 59,  90,  120, 151, 181, 212,
...
long FATPROC date_dos2unix(unsigned short time, unsigned short date)
{
	...
	return (time & 31) * 2 + 60 * ((time >> 5) & 63) +
	       (time >> 11) * 3600L +
	       86400L * ((date & 31) - 1 + day_n[month] + (year / 4) +
			 year * 365 - ((year & 3) == 0 && month < 2 ? 1 : 0) +
			 3653) +
	       tz_offset * 3600L;
}
...
void FATPROC date_unix2dos(long unix_date, unsigned short *time,
			   unsigned short *date)
{
...
ino_t FATPROC msdos_get_entry(struct inode *dir, loff_t *pos,
			      struct buffer_head **bh,
			      struct msdos_dir_entry **de)
{
	...
	while (1) {
		offset = *pos;
		if ((sector = msdos_smap(
			     dir, (sector_t)(*pos >> SECTOR_BITS(dir)))) == -1)
			return -1;
		if (!sector)
			return -1; /* FAT error ... */
		*pos += sizeof(struct msdos_dir_entry);
		...
		if (!(*bh = msdos_sread(dir->i_sb, sector, &data)))
			continue;
		*de = (struct msdos_dir_entry *)((char *)data +
						 (offset &
						  (SECTOR_SIZE(dir) - 1)));
		...
		return (sector << MSDOS_DPS_BITS(dir)) +
		       ((offset & (SECTOR_SIZE(dir) - 1)) >> MSDOS_DIR_BITS);
	}
}
...
int FATPROC msdos_scan(struct inode *dir, char *name,
		       ...
		       struct msdos_dir_entry **res_de, ino_t *ino)
{
	...
	*res_bh = NULL;
	while ((*ino = msdos_get_entry(dir, &pos, res_bh, &de)) != (ino_t)-1) {
		if (name) {
			if (de->name[0] &&
			    (unsigned char)de->name[0] != DELETED_FLAG &&
			    !(de->attr & ATTR_VOLUME) &&
			    !strncmp(de->name, name, MSDOS_NAME))
				break;
		} else if (!de->name[0] ||
			   (unsigned char)de->name[0] == DELETED_FLAG) {
...
static cluster_t FATPROC raw_found(struct super_block *sb, cluster_t sector,
				   const char *name, cluster_t number,
				   ino_t *ino)
{
	struct buffer_head *bh;
...
#endif
			if (name) {
				if (strncmp(data[entry].name, name,
					    MSDOS_NAME) != 0)
					continue;
				start = data[entry].start |
					((unsigned long)starthi << 16);
			} else {
				if ((unsigned char)data[entry].name[0] ==
					    DELETED_FLAG ||
				    data[entry].start !=
				       ...
				       ino_t *ino)
{
	...
	for (count = 0; count < MSDOS_SB(sb)->dir_entries / MSDOS_DPS_SB(sb);
	     count++) {
		if ((cluster = raw_found(
			     sb, (cluster_t)(MSDOS_SB(sb)->dir_start + count),
			     name, number, ino)) >= 0)
			break;
	}
	return cluster;
...
static cluster_t FATPROC raw_scan_nonroot(register struct super_block *sb,
					  cluster_t start, const char *name,
					  cluster_t number, ino_t *ino)
{
	int count;
	cluster_t cluster;
...
ino_t FATPROC msdos_parent_ino(register struct inode *dir, int locked)
{
	...
	if ((curr = raw_scan(dir->i_sb, dir->u.msdos_i.i_start, MSDOS_DOTDOT,
			     0L, NULL)) < 0) {
	} else if (!curr)
		this = MSDOS_ROOT_INO;
	else {
		if ((prev = raw_scan(dir->i_sb, curr, MSDOS_DOTDOT, 0L, NULL)) <
		    0) {
