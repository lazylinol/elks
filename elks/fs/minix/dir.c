...
static size_t minix_dir_read(struct inode *inode, struct file *filp, char *buf,
			     size_t count)
{
	return -EISDIR;
}

static int minix_readdir(struct inode *inode, register struct file *filp,
			 ...
			 char *dirent, filldir_t filldir)
{
	...
	if (((unsigned short)filp->f_pos) & (info->s_dirsize - 1))
		return -EBADF;
	while (filp->f_pos < (loff_t)inode->i_size) {
		offset = (unsigned short)filp->f_pos & (BLOCK_SIZE - 1);
		bh = minix_bread(
			inode, (block_t)((filp->f_pos) >> BLOCK_SIZE_BITS), 0);
		if (!bh) {
		...
		do {
			...
			if (de->inode) {
				filldir(dirent, de->name,
					strnlen(de->name, info->s_namelen),
					filp->f_pos, de->inode);
