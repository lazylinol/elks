...
static int V1_trunc_indirect(register struct inode *inode, unsigned int offset,
			     unsigned short *p)
{
...
repeat:
	...
	for (j = INDIRECT_BLOCK(offset); j < 512; j++) {
		if (j < 0)
			j = 0;
		else if (j < INDIRECT_BLOCK(offset))
			goto repeat;
...
static int V1_trunc_dindirect(register struct inode *inode, unsigned int offset,
			      unsigned short *p)
{
	int i;
	unsigned short tmp;
...
void minix_truncate(register struct inode *inode)
{
	...
	while (1) {
		...
		retry |= V1_trunc_indirect(inode, 7,
					   &inode->u.minix_i.i_zone[7]);
		retry |= V1_trunc_dindirect(inode, 7 + 512,
					    &inode->u.minix_i.i_zone[8]);
		if (!retry)
			break;
		schedule();
	}
