...
static unsigned short count_used(kdev_t dev, block_t map[],
...
unsigned short minix_count_free_blocks(register struct super_block *sb)
{
	return (sb->u.minix_sb.s_nzones -
		count_used(sb->s_dev, sb->u.minix_sb.s_zmap,
...
unsigned short minix_count_free_inodes(register struct super_block *sb)
{
	return (sb->u.minix_sb.s_ninodes -
		count_used(sb->s_dev, sb->u.minix_sb.s_imap,
...
void minix_free_block(register struct super_block *sb, unsigned short block)
{
	...
	if (block < sb->u.minix_sb.s_firstdatazone ||
		...
		bh = get_map_block(sb->s_dev,
				   sb->u.minix_sb.s_zmap[zone >> 13]);
		if (!bh)
			s = "null zmap";
		else {
			map_buffer(bh);
...
repeat:
	...
	for (i = 0; i < sb->u.minix_sb.s_zmap_blocks; i++) {
		if (i > 0)
			unmap_brelse(bh);
		if ((bh = get_map_block(sb->s_dev, sb->u.minix_sb.s_zmap[i])) !=
		    NULL) {
			map_buffer(bh);
			if ((j = find_first_zero_bit((void *)bh->b_data,
						     8192)) < 8192)
				break;
		}
	}
...
void minix_free_inode(register struct inode *inode)
{
	...
	if (!inode->i_dev)
		...
		s = "no sb\n";
	else if ((int)inode->i_ino == 0)
		s = "inode 0\n";
	else if ((unsigned int)inode->i_ino > inode->i_sb->u.minix_sb.s_ninodes)
		s = "nonexistent inode\n";
	else if (!(bh = get_map_block(
			   inode->i_sb->s_dev,
			   inode->i_sb->u.minix_sb
				   .s_imap[(unsigned int)inode->i_ino >> 13])))
		s = "nonexistent imap\n";
	if (s) {
		printk("free_inode: ");
		printk(s, n);
...
struct inode *minix_new_inode(struct inode *dir, __u16 mode)
{
	...
	for (i = 0; i < sb->u.minix_sb.s_imap_blocks; i++) {
		if (i > 0)
			unmap_brelse(bh);
		if ((bh = get_map_block(sb->s_dev, sb->u.minix_sb.s_imap[i])) !=
		    NULL) {
			map_buffer(bh);
			if ((j = find_first_zero_bit((void *)bh->b_data,
						     8192)) < 8192)
