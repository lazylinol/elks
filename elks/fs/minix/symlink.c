...
static int minix_follow_link(struct inode *dir, register struct inode *inode,
	...
	if (!inode)
		...
		*res_inode = inode;
		...
		if (!bh)
			...
			ds = *pds;
			*pds = kernel_ds;
			error = open_namei(bh->b_data, flag, mode, res_inode,
					   dir);
			*pds = ds;
			/* current-> */ link_count--;
			unmap_brelse(bh);
...
static int minix_readlink(register struct inode *inode, char *buffer,
			  size_t buflen)
{
	register struct buffer_head *bh;
	size_t len;
	...
	if (!S_ISLNK(inode->i_mode))
		...
		if (!bh)
			...
			map_buffer(bh);
			if ((len = strnlen(bh->b_data, 1023)) > buflen)
				len = buflen;
