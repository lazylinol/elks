...
static int namecompare(size_t len, size_t max, const char *name,
		       register char *buf)
{
	return ((len < max) && (buf[len] != 0)) ? 0 :
							!fs_memcmp(name, buf, len);
}
 ...
 */
static int minix_match(size_t len, const char *name,
		       register struct minix_dir_entry *de, size_t minixlen)
{
	if (!de->inode) {
		return 0;
	}
	...
	*res_dir = NULL;
	...
	do {
		...
		if (!offset) {
			unmap_brelse(bh);
minix_find:
			bh = minix_bread(dir, (__u16)(bo >> BLOCK_SIZE_BITS),
					 0);
			if (!bh) {
				continue;
			}
		...
		if (minix_match(namelen, name,
				(struct minix_dir_entry *)(bh->b_data + offset),
				info->s_namelen)) {
			*res_dir =
				(struct minix_dir_entry *)(bh->b_data + offset);
			return bh;
...
int minix_create(register struct inode *dir, char *name, size_t len, int mode,
		 struct inode **result)
{
	...
	if (!(inode = minix_new_inode(dir, (__u16)mode)))
		error = -ENOSPC;
	else {
		/*----------------------------------------------------------------------*/
		/*----------------------------------------------------------------------*/
...
int minix_mknod(register struct inode *dir, char *name, size_t len, int mode,
		int rdev)
{
	int error;
...
int minix_mkdir(register struct inode *dir, char *name, size_t len, int mode)
{
	...
	inode->i_nlink++;
	strcpy(de->name, ".");
	de = (struct minix_dir_entry *)(((char *)de) +
...
static int empty_dir(register struct inode *inode)
{
	...
	if ((unsigned short)(inode->i_size) & (dirsize - 1))
		goto bad_dir;
	if (inode->i_size < (__u32)(dirsize << 1))
		goto bad_dir;
	...
	while ((bo += dirsize) < inode->i_size) {
		...
		if (!offset) {
			unmap_brelse(bh);
			bh = minix_bread(inode, (__u16)(bo >> BLOCK_SIZE_BITS),
					 0);
			if (!bh)
				continue;
			map_buffer(bh);
	...
	if (bh) {
		...
		if ((inode = iget(dir->i_sb, (ino_t)de->inode))) {
			if (((dir->i_mode & S_ISVTX) && !suser() &&
			    ...
			    (inode ==
			     dir) /* we may not delete ".", but "../dir" is ok */
			)
				;
...
int minix_symlink(struct inode *dir, char *name, size_t len, char *symname)
{
	...
	if (!bh)
		goto symlink1;
	if ((error = strlen_fromfs(symname, 1023)) > 1023)
		error = 1023;
	       ...
	       register struct inode *oldinode)
{
	...
	if (S_ISDIR(oldinode->i_mode))
		...
		unmap_brelse(bh);
		error = -EEXIST;
	} else if (!(error = minix_add_entry(dir, name, len,
					     oldinode->i_ino))) {
		oldinode->i_nlink++;
