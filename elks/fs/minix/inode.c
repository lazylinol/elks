...
static unsigned short map_iblock(register struct inode *, block_t, block_t,
				 int);
static unsigned short map_izone(register struct inode *, block_t, int);
static int minix_set_super_state(struct super_block *sb, int notflags,
				 int state);
...
void minix_write_super(register struct super_block *sb)
{
	...
	if (!(sb->s_flags & MS_RDONLY))
		minix_set_super_state(
			sb, ~MINIX_VALID_FS,
			0); /* unset fs checked flag, commit superblock*/
	sb->s_dirt = 0;
}
...
void minix_put_super(register struct super_block *sb)
{
	...
	if (!(sb->s_flags & MS_RDONLY))
		minix_set_super_state(
			sb, 0,
			sb->u.minix_sb.s_mount_state); /* set original fs state*/
	unlock_super(sb);
	sb->s_dev = 0;
}

static struct super_operations minix_sops = {
	minix_read_inode,  minix_write_inode, minix_put_inode, minix_put_super,
	minix_write_super, minix_remount,     minix_statfs
};

static void minix_mount_warning(register struct super_block *sb,
				const char *prefix)
{
	if ((sb->u.minix_sb.s_mount_state &
	     (MINIX_VALID_FS | MINIX_ERROR_FS)) != MINIX_VALID_FS)
...
int minix_remount(register struct super_block *sb, int *flags, char *data)
{
	...
	if (*flags & MS_RDONLY) {
		...
		minix_set_super_state(
			sb, 0,
			sb->u.minix_sb
				.s_mount_state); /* reset original fs state and commit superblock*/
		sb->s_dirt = 0;
	} else {
		/* Mount a partition which is read-only, read-write*/
		debug_sup("MINIX remount RO to RW\n");
		sb->u.minix_sb.s_mount_state = minix_set_super_state(
			sb, ~MINIX_VALID_FS, 0); /* unset fs checked flag*/
		sb->s_dirt = 1;
...
struct super_block *minix_read_super(register struct super_block *s, char *data,
				     int silent)
{
	struct buffer_head *bh;
	struct minix_super_block *ms;
	...
	if (block !=
	    2 + s->u.minix_sb.s_imap_blocks + s->u.minix_sb.s_zmap_blocks) {
		msgerr = err2;
		goto err_read_super_1;
	}
	...
	if (!(s->s_flags & MS_RDONLY)) {
		s->s_dirt =
			1; /* will unset MINIX_VALID_FS flag in write_super */
...
static unsigned short map_iblock(struct inode *inode, block_t i, block_t block,
				 int create)
{
			   ...
			   int create)
{
	...
	if (i != 0) {
		...
		i = map_iblock(inode, (block_t)i, (block_t)(block >> 9),
			       create);
...
map1:
	 ...
	 */
		if (i != 0)
			...
			i = map_iblock(inode, (block_t)i,
				       (block_t)(block & 511), create);
	}
	return i;
}

struct buffer_head *minix_getblk(register struct inode *inode, block_t block,
				 int create)
{
...
struct buffer_head *minix_bread(struct inode *inode, unsigned short block,
				int create)
{
	register struct buffer_head *bh;
...
void minix_set_ops(register struct inode *inode)
{
	static unsigned char tabc[] = {
		0, 0, 0, 0, 1, 0, 0, 0, 2, 0, 3, 0, 0, 0, 0, 0,
	};
...
struct file_system_type minix_fs_type = { minix_read_super, FST_MINIX };
