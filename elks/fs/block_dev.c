...
size_t block_read(struct inode *inode, struct file *filp, char *buf,
		  size_t count)
{
	 ...
	 */
		...
		chars = BLOCK_SIZE -
			(((size_t)(filp->f_pos)) & (BLOCK_SIZE - 1));
		if (chars > count)
			chars = count;
		if (bh) {
			if (!readbuf(bh)) {
			...
			fmemsetb(buf, current->t_regs.ds, 0, chars);
		buf += chars;
		filp->f_pos += chars;
		read += chars;
		count -= chars;
...
size_t block_write(struct inode *inode, struct file *filp, char *buf,
		   size_t count)
{
#if defined(CONFIG_MINIX_FS) || defined(CONFIG_BLK_DEV_CHAR)
	size_t chars, offset;
...
static int blk_rw(struct inode *inode, register struct file *filp, char *buf,
		  size_t count, int wr)
{
	register struct buffer_head *bh;
	size_t chars, offset;
	 ...
	 */
		bh = getblk(inode->i_rdev,
			    (block_t)(filp->f_pos >> BLOCK_SIZE_BITS));
	     ...
	     *      Alter buffer, mark dirty
	     */
			xms_fmemcpyb(buffer_data(bh) + offset, buffer_seg(bh),
	     ...
	     */
			xms_fmemcpyb(buf, current->t_regs.ds,
				     buffer_data(bh) + offset, buffer_seg(bh),
				     chars);
		}
		/*
...
size_t blk_write(struct inode *inode, struct file *filp, char *buf,
		 size_t count)
{
