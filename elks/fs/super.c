...
static struct super_block *read_super(kdev_t dev, int t, int flags, char *data,
				      int silent)
{
	register struct super_block *s;
...
static int do_remount_sb(register struct super_block *sb, int flags, char *data)
{
	...
	sb->s_flags = (unsigned short int)((sb->s_flags & ~MS_RMT_MASK) |
					   (flags & MS_RMT_MASK));
...
int sys_mount(char *dev_name, char *dir_name, int type, int flags)
{
	...
	if (!S_ISBLK(inodep->i_mode))
		...
		retval = open_filp((mode_t)((flags & MS_RDONLY) ? 1 : 3),
				   inodep, &filp);
	if (retval) {
		iput(inodep);
		return retval;
...
retry_floppy:
	retval = open_filp(((root_mountflags & MS_RDONLY) ? 0 : 2), d_inode,
			   &filp);
	if (retval == -EROFS) {
