...
#define SET_COUNT(i)                                      \
	if (--nr_free_inodes < 0) {                       \
		panic("get_empty_inode: bad free count"); \
	}
#else
...
static void list_inode_status(void)
{
	...
	do {
		if (inode->i_count || inode->i_dev || inode->i_dirt) {
			...
			printk("\n#%2d: dev %p inode %5lu cnt %2d %c %06o %s",
			       i, inode->i_dev, (unsigned long)inode->i_ino,
			       inode->i_count, inode->i_dirt ? 'D' : ' ',
			       inode->i_mode,
...
#if defined(CHECK_FREECNTS) && DEBUG_EVENT
	debug_setcallback(0,
			  list_inode_status); /* ^N will generate inode list */
#endif
}
	...
	if (inode) {
		...
		if (!inode->i_count) {
			printk("iput: trying to free free inode dev %D inode %lu mode 0%06o\n",
			       inode->i_rdev, (unsigned long)inode->i_ino,
			       inode->i_mode);
			return;
		}
#if UNUSED
...
static void set_ops(register struct inode *inode)
{
	static unsigned char tabc[] = {
		0, 1, 2, 0, 0, 0, 3, 0, 0, 0, 0, 0, 4, 0, 0, 0,
	};
			   ...
			   register struct iattr *attr)
{
	...
	if ((attr->ia_valid & ATTR_UID) &&
	    (current->euid != inode->i_uid || attr->ia_uid != inode->i_uid) &&
	    !suser()) {
		return -EPERM;
	}
