...
static size_t romfs_read(struct inode *i, struct file *f, char *buf, size_t len)
{
	size_t count;
	...
	while (1) {
		...
		fmemcpyb(buf, current->t_regs.ds, (char *)(int)f->f_pos,
			 i->u.romfs.seg, len);
...
static int romfs_readdir(struct inode *i, struct file *f, void *dirent,
			 filldir_t filldir)
{
	int res;
			...
			struct inode **result)
{
	...
	while (1) {
		...
		while (offset < dir->i_size) {
			...
			if (len2 == len1) {
				if (!fmemcmpb((char *)offset + 3, seg_i,
					      (void *)name, current->t_regs.ds,
					      len2)) {
...
static mode_t romfs_modemap[] = { S_IFREG, S_IFDIR, S_IFCHR, S_IFBLK, S_IFLNK };

static struct inode_operations *romfs_inode_ops[] = {
	&romfs_inode_operations, &romfs_inode_operations,
...
static struct super_operations romfs_super_ops = { romfs_read_inode,
						   NULL, /* write inode */
						   NULL, /* put inode */
						   romfs_put_super,
						   NULL, /* write super */
...
static struct super_block *romfs_read_super(struct super_block *s, void *data,
					    int silent)
{
	...
	while (1) {
		...
		if (fmemcmpw((void *)ROMFS_MAGIC_STR, kernel_ds, 0,
			     CONFIG_ROMFS_BASE, ROMFS_MAGIC_LEN >> 1)) {
		...
		fmemcpyw(&rsm, kernel_ds, 0, CONFIG_ROMFS_BASE,
			 sizeof(struct romfs_super_mem) >> 1);
...
struct file_system_type romfs_fs_type = { romfs_read_super, FST_ROMFS };
