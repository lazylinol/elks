...
int nr_xms_bufs =
	CONFIG_FS_NR_XMS_BUFFERS; /* override with /bootopts xmsbuf= */
#endif
...
static struct buffer_head
	*bh_lru; /* least recently used - reused/flushed first */
static struct buffer_head
	*bh_llru; /* most recently used - for finding a buffer */
...
static struct buffer_head
	*L1map[MAX_NR_MAPBUFS];	 /* L1 indexed pointer to L2 buffer */
	...
	do {
		...
		if (isinuse || bh->b_data) {
			...
			printk("\n#%3d: buf %3d blk/dev %5ld/%p %c%c%c %smapped L%02d %d count %d",
			       ...
			       ebh->b_locked ? 'L' : ' ',
			       ebh->b_dirty ? 'D' : ' ',
			       ebh->b_uptodate ? 'U' : ' ', j ? "  " : "un", j,
			       ebh->b_mapcount, ebh->b_count);
		}
		i++;
		if (isinuse)
			inuse++;
	} while ((bh = ebh->b_prev_lru) != NULL);
	printk("\nTotal L2 buffers inuse %d/%d (%d free)", inuse, nr_bh,
	       nr_free_bh);
	printk(", %dk L1 (map %u, unmap %u remap %u)\n", nr_map_bufs, map_count,
	       unmap_count, remap_count);
}
#endif

int INITPROC buffer_init(void)
{
	if (nr_map_bufs > MAX_NR_MAPBUFS)
		nr_map_bufs = MAX_NR_MAPBUFS;
...
#endif
#ifdef CONFIG_FAR_BUFHEADS
	if (bufs_to_alloc > 2975)
		bufs_to_alloc = 2975; /* max 64K far bufheads @22 bytes*/
#else
	if (bufs_to_alloc > 256)
		bufs_to_alloc = 256; /* protect against high XMS value*/
#endif

	printk("%d %s buffers (%dK ram), %dK cache, %d req hdrs\n",
	       bufs_to_alloc, xms_enabled ? "xms" : "ext", bufs_to_alloc,
	       nr_map_bufs, NR_REQUEST);
#else
...
#if defined(CHECK_FREECNTS) && DEBUG_EVENT
	debug_setcallback(
		1, list_buffer_status); /* ^O will generate buffer list */
#endif

	if (!(L1buf = heap_alloc(nr_map_bufs * BLOCK_SIZE,
	...
	do {
		...
		if (xms_enabled) {
			ramdesc_t xmsseg =
				xms_alloc((long_t)nbufs << BLOCK_SIZE_BITS);
			add_buffers(nbufs, 0, xmsseg);
		} else
#endif
		{
			segment_s *extseg =
				seg_alloc(nbufs << (BLOCK_SIZE_BITS - 4),
...
static void sync_buffers(kdev_t dev, int wait)
{
	...
	do {
		...
		if (ebh->b_locked) {
			debug_blk(
				"SYNC: dev %p buf %d block %ld LOCKED mapped %d skipped %d data %04x\n",
				ebh->b_dev, buf_num(bh), ebh->b_blocknr,
				ebh->b_mapcount, !wait, bh->b_data);
			if (!wait)
				continue;
			wait_on_buffer(bh);
		}
         ...
         */
		debug_blk(
			"sync: dev %p write buf %d block %ld count %d dirty %d\n",
		...
		if ((bh = ebh->b_next_lru) == NULL) {
			...
			if (++pass > 1) {
				for (i = 0; i < nr_map_bufs; i++) {
					brelseL1_index(
						i,
						1); /* release if not mapcount or locked */
				}
			}
			bh = bh_lru;
		}
...
#ifdef CHECK_BLOCKIO
	if (ebh->b_mapcount)
		panic("get_free_buffer"); /* mapped buffer reallocated */
#endif
	put_last_lru(bh);
	ebh->b_uptodate = 0;
	ebh->b_count = 1;
...
static struct buffer_head *find_buffer(kdev_t dev, block32_t block)
{
	...
	do {
		...
		if (ebh->b_blocknr == block && ebh->b_dev == dev)
			break;
	} while ((bh = ebh->b_prev_lru) != NULL);
	return bh;
}
...
{
#if defined(CONFIG_FS_XMS_INT15) || (!defined(CONFIG_FS_EXTERNAL_BUFFER) && \
				     !defined(CONFIG_FS_XMS_BUFFER))
#define FORCEMAP 1
#else
#define FORCEMAP 0
#endif
...
void map_buffer(struct buffer_head *bh)
{
	...
	if (ebh->b_uptodate)
		xms_fmemcpyw(bh->b_data, kernel_ds, 0, ebh->b_L2seg,
			     BLOCK_SIZE / 2);
	map_count++;
...
void brelseL1_index(int i, int copyout)
{
	...
	if (copyout && ebh->b_uptodate && bh->b_data) {
		xms_fmemcpyw(0, ebh->b_L2seg, bh->b_data, kernel_ds,
			     BLOCK_SIZE / 2);
		unmap_count++;
	}
	bh->b_data = 0;
