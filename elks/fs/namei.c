...
int permission(register struct inode *inode, int mask)
{
	...
	if ((mask & MAY_WRITE) && IS_RDONLY(inode) && !S_ISCHR(inode->i_mode) &&
	    !S_ISBLK(inode->i_mode)) /* allow writable devices*/
		error = -EROFS;
...
static int dir_namei(const char *pathname, size_t *namelen, const char **name,
		     register struct inode *base, struct inode **res_inode)
{
	const char *thisname;
	struct inode *inode;
	struct inode *baser;
...
static void save_path(struct inode *inode, const char *pathname)
{
	...
	if (!inode->i_path[0])
		fmemcpyb(inode->i_path, kernel_ds, (void *)pathname,
			 current->t_regs.ds, sizeof(inode->i_path));
#endif
}
 ...
 */
int namei(const char *pathname, register struct inode **res_inode, int dir,
	  int perm)
{
	...
	if (flag & O_CREAT) {
		...
		if (!error) {
			...
			if (!iop || !iop->create)
				...
				error = iop->create(dirp, basename, namelen,
						    mode, res_inode);
				up(&dirp->i_sem);
				iput(dirp);
				goto onamei_end;
...
int do_mknod(char *pathname, int offst, int mode, dev_t dev)
{
	...
	if (!error) {
		...
		if (!namelen) {
			if (offst ==
			    (int)offsetof(struct inode_operations, link))
				error = -EPERM;
			else
				error = -ENOENT;
		}
		/*      else if (IS_RDONLY(dirp)) error = -EROFS; */
			  ...
			  (int)offsetof(struct inode_operations, link)) &&
			 (dirp->i_dev != ((struct inode *)mode)->i_dev))
			error = -EXDEV;
		else if (!(error = permission(dirp, MAY_WRITE | MAY_EXEC))) {
			iop = dirp->i_op;
			if (!iop ||
				...
				dirp->i_count++;
				down(&dirp->i_sem);
				error = (offst != (int)offsetof(
							  struct inode_operations,
							  mknod) ?
						       op(dirp, basename, namelen,
						    mode) :
						       op(dirp, basename, namelen,
						    mode, dev));
				up(&dirp->i_sem);
...
int sys_mknod(char *pathname, int mode, dev_t dev)
{
	...
	return do_mknod(pathname, offsetof(struct inode_operations, mknod),
			mode, dev);
#endif
}
...
int sys_mkdir(char *pathname, int mode)
{
	...
	return -EROFS;
#else
	return do_mknod(pathname, offsetof(struct inode_operations, mkdir),
			(mode & 0777) | S_IFDIR, 0);
#endif
}
...
int sys_symlink(char *oldname, char *pathname)
{
	...
	return -EROFS;
#else
	return do_mknod(pathname, offsetof(struct inode_operations, symlink),
			(int)oldname, 0);
#endif
}
...
int sys_link(char *oldname, char *pathname)
{
	...
	if (!error)
		error = do_mknod(pathname,
				 offsetof(struct inode_operations, link),
				 (int)oldinode, 0);
	return error;
#endif
}
