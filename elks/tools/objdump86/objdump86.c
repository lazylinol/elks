...
struct ar_hdr {
	char ar_name[16], ar_date[12], ar_uid[6], ar_gid[6], ar_mode[8],
...
long read_arheader(archentry) char *archentry;
{
	char *endptr;
...
void do_module(fname, archive) char *fname;
char *archive;
{
	int modno, i;
	...
	case 0: /* as86 object file */
		...
		for (modno = 1; modno <= sections; modno++) {
			...
			if (display_mode == 0)
				printf("text\tdata\tbss\tdec\thex\tfilename\n");
			if (display_mode != 2) {
				printf("%ld\t%ld\t%ld\t%ld\t%lx\t", size_text,
				       size_data, size_bss,
	...
	case 2: /* V7 executable */
		...
		case 1:
			...
			break;
		case 2:
			error("Symbol table not supported for v7");
			exit(1);
			break;
		}
		break;
...
void read_databytes()
{
	...
	for (;;) {
		...
		case 0x80: /* Relocator - simple */
			l = get_sized(relsize);
			printf("%s SEG%x%s%s", relstr[relsize], (ch & 0xF),
			       (ch & 0x20) ? "-PC" : "",
			       (ch & 0x10) ? "+?" : "");
			if (l)
				printf("+0x%04lx", l);
		...
		case 0xC0: /* Relocator - symbol relative */
			...
			printf("%s %s%s%s", relstr[relsize], symnames[i],
			       (ch & 0x20) ? "-PC" : "",
			       (ch & 0x18) ? "+?" : "");
			if (l)
				printf("+0x%04lx", l);
...
void dump_aout()
{
	static char *cpu[] = { "unknown", "8086", "m68k",
			       "ns16k",	  "i386", "sparc" };
	static char *byteord[] = { "LITTLE_ENDIAN", "(2143)", "(3412)",
				   "BIG_ENDIAN" };
	int i;
	long l;
...
void size_v7()
{
	...
	printf("%ld\t%ld\t%ld\t%ld\t%lx\t%s\n", header[1], header[2], header[3],
	       header[1] + header[2] + header[3],
	       header[1] + header[2] + header[3], ifname);
