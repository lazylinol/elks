...
#define ELKS_BPB_NumTracks \
	(SECT_SIZE - 512 + 0x1F7) /* offset of number of tracks (word)*/
#define ELKS_BPB_SecPerTrk \
	(SECT_SIZE - 512 + 0x1F9) /* offset of sectors per track (byte)*/
#define ELKS_BPB_NumHeads \
	(SECT_SIZE - 512 + 0x1FA) /* offset of number of heads (byte)*/
...
int main(int argc, char **argv)
{
	...
	if (argc < 2 || argc > 6)
		fatalmsg(
			"Usage: %s <image> [-F] [-K] [-S{m,f}] [-{B,P}<sectors>,<heads>[,<tracks>]] [<input_boot_image>]\n",
			argv[0]);
	...
	while (argv[1] && argv[1][0] == '-') {
		if (argv[1][1] == 'B') {
			...
			if (numcommas(&argv[1][2]) == 2) { /* tracks specified*/
				if (sscanf(&argv[1][2], "%d,%d,%d", &SecPerTrk,
					   &NumHeads, &NumTracks) != 3)
					fatalmsg(
						"Invalid -B<sectors>,<heads>,<tracks> option\n");
				printf("Updating BPB to %d sectors, %d heads, %d tracks\n",
				       SecPerTrk, NumHeads, NumTracks);
			} else {
				if (sscanf(&argv[1][2], "%d,%d", &SecPerTrk,
					   &NumHeads) != 2)
					fatalmsg(
						"Invalid -B<sectors>,<heads> option\n");
			...
			if (sscanf(&argv[1][2], "%d,%d,%d", &SecPerTrk,
				   &NumHeads, &NumTracks) != 3)
				fatalmsg(
					"Invalid -P<sectors>,<heads>,<cylinders> option\n");
			...
			argc--;
			   ...
			   'S') { /* update start sector in ELKS boot sector*/
			if (argv[1][2] == 'm')
				opt_update_start_sector_minix = 1;
			if (argv[1][2] == 'f')
				opt_update_start_sector_fat = 1;
	...
	if (opt_new_bootsect) {
		...
		if (blk[SECT_SIZE - 2] != 0x55 ||
		    blk[SECT_SIZE - 1] != 0xaa) /* 510, 511*/
			fprintf(stderr,
				"Warning: '%s' may not be valid bootable sector\n",
				infile);

		if (fwrite(blk, 1, count, ofp) != count)
			die("fwrite(%s)", infile);

		if (opt_update_start_sector_minix ||
		    opt_update_start_sector_fat) {
			...
			if (opt_update_start_sector_minix) {
				blk[MINIX_SectOffset] =
					(unsigned char)StartSector;
				blk[MINIX_SectOffset + 1] =
					(unsigned char)(StartSector >> 8);
				blk[MINIX_SectOffset + 2] =
					(unsigned char)(StartSector >> 16);
				blk[MINIX_SectOffset + 3] =
					(unsigned char)(StartSector >> 24);
			}
			if (opt_update_start_sector_fat) {
				blk[FAT_BPB_SectOffset] =
					(unsigned char)StartSector;
				blk[FAT_BPB_SectOffset + 1] =
					(unsigned char)(StartSector >> 8);
				blk[FAT_BPB_SectOffset + 2] =
					(unsigned char)(StartSector >> 16);
				blk[FAT_BPB_SectOffset + 3] =
					(unsigned char)(StartSector >> 24);
			}
