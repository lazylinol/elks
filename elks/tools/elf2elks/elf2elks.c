...
static void error_1(const char *fmt, va_list ap)
{
...
static void error_with_errno(const char *fmt, ...)
{
	int err = errno;
	va_list ap;
	va_start(ap, fmt);
...
static void error_with_elf_msg(const char *fmt, ...)
{
	int err = elf_errno();
	va_list ap;
	va_start(ap, fmt);
...
static void error_with_help(const char *fmt, ...)
{
	va_list ap;
	va_start(ap, fmt);
	error_1(fmt, ap);
...
static void parm_uint16(int argc, char **argv, uint16_t *pvalue, int *pi)
{
	char *ep;
	int i = *pi + 1;
	*pi = i;
	if (i >= argc)
		error_with_help("expected integer argument after `%s'",
				argv[i - 1]);
...
static void parse_args(int argc, char **argv)
{
		...
		if (arg[0] == '-') {
			if (strcmp(arg + 1, "v") == 0)
				...
				if (strcmp(arg + 2, "tiny") == 0)
					...
					parm_uint16(argc, argv, &total_data,
						    &i);
					...
					error_with_help("unknown option `%s'",
							arg);
			} else
	...
	if (aout_seg_given ^ data_seg_given)
		error_with_help(
			"cannot specify only --aout-seg or only --data-seg");

	if (total_data) {
		if (chmem)
			error_with_help(
				"cannot specify both --total-data and --chmem");
		if (stack)
			error_with_help(
				"cannot specify both --total-data and --stack");
		if (heap)
			error_with_help(
				"cannot specify both --total-data and --heap");
	}

	if (chmem) {
		if (stack)
			error_with_help(
				"cannot specify both --chmem and --stack");
		if (heap)
			error_with_help(
				"cannot specify both --chmem and --heap");
	}
}

static void set_scn(Elf_Scn **pscn, const Elf32_Shdr **psh, Elf_Scn *scn,
...
static bool in_scn_p(uint32_t addr, const Elf32_Shdr *shdr)
{
	return shdr && addr >= shdr->sh_addr &&
	       addr < shdr->sh_addr + shdr->sh_size;
}

static void check_scn_overlap(const Elf32_Shdr *shdr1, const char *nature1,
			      const Elf32_Shdr *shdr2, const char *nature2)
{
	if (!shdr1 || !shdr2)
		return;

	if (in_scn_p(shdr1->sh_addr, shdr2) || in_scn_p(shdr2->sh_addr, shdr1))
		error("%s and %s sections overlap!", nature1, nature2);
}

static void input_for_header(void)
{
	...
	if (!elf)
		error_with_elf_msg("cannot open input file `%s' as ELF",
				   file_name);
	...
	for (sidx = 1; sidx < num_scns; ++sidx) {
		...
		if (!scn)
			error_with_elf_msg("cannot read ELF section %#zx",
					   sidx);

		shdr = elf32_getshdr(scn);
		if (!shdr)
			error_with_elf_msg(
				"cannot read ELF section %#zx header", sidx);

		switch (shdr->sh_type) {
		case SHT_REL:
			if (shdr->sh_info == 0)
				set_scn(&rel_dyn, &rel_dyn_sh, scn, shdr,
					"dynamic relocations", sidx);
		...
		case SHT_PROGBITS:
			...
			if (!name)
				error_with_elf_msg(
					"cannot read ELF section %#zx name",
					sidx);

			if (strcmp(name, ".text") == 0)
				set_scn(&text, &text_sh, scn, shdr, "text",
					sidx);
			else if (strcmp(name, ".fartext") == 0)
				set_scn(&ftext, &ftext_sh, scn, shdr,
					"far text", sidx);
			else if (strcmp(name, ".data") == 0)
				set_scn(&data, &data_sh, scn, shdr, "data",
					sidx);
			else if (shdr->sh_size != 0 &&
				 (shdr->sh_flags & SHF_ALLOC) != 0)
				error("stray SHT_PROGBITS SHF_ALLOC section %#zx `%s'",
				      sidx, name);
		...
		case SHT_NOBITS:
			...
			if (strcmp(name, ".bss") == 0)
				set_scn(&bss, &bss_sh, scn, shdr, "BSS", sidx);
				 ...
				 (shdr->sh_flags & SHF_ALLOC) != 0)
				error("stray SHT_NOBITS SHF_ALLOC section %#zx `%s'",
				      sidx, name);

			break;

		default:; /* ignore other types of sections */
		}
	}
	...
	if (tiny) {
		...
		if (text) {
			if (data && text_sh->sh_addr + text_sh->sh_size !=
					    data_sh->sh_addr)
				error("data segment must come right after text in tiny model!");

			if (!data && bss &&
			    text_sh->sh_addr + text_sh->sh_size !=
				    bss_sh->sh_addr)
				error("data segment must come right after text in tiny model!");
		}
	}
	...
	INFO("%" PRIu32 " text reloc(s)., %" PRIu32 " far text reloc(s)., "
	     "%" PRIu32 " data reloc(s).",
	     text_n_rels, ftext_n_rels, data_n_rels);
}

static void create_symtab(void)
{
	FILE *infp;
	FILE *outfp;
	char command[256];
	...
	while (fgets(buf, sizeof(buf), infp) != NULL) {
		...
		if (sscanf(buf, "%x %c %s", &addr, &type, name) == 3) {
			int istext = text_start &&
				     (addr >= text_start && addr < text_end);
			int isftext = ftext_start &&
				      (addr >= ftext_start && addr < ftext_end);
			int isdata = data_start &&
...
static void output(const void *buf, size_t n)
{
	ssize_t r;
...
static void output_header(void)
{
   ...
   */
	...
	if (data_sh) {
		...
		if (bss_sh)
			mh.bseg += bss_sh->sh_addr - data_sh->sh_addr -
				   data_sh->sh_size;
	}
	mh.entry = entry;
	...
	if (!romable && ((ftext_sh && ftext_sh->sh_size) || ftext_n_rels)) {
		...
		esuph1.trsize =
			(uint32_t)text_n_rels * sizeof(struct minix_reloc);
		esuph1.drsize =
			(uint32_t)data_n_rels * sizeof(struct minix_reloc);
		esuph2.ftseg = ftext_sh->sh_size;
		esuph2.ftrsize =
			(uint32_t)ftext_n_rels * sizeof(struct minix_reloc);
		output(&mh, sizeof mh);
...
static void convert_reloc(struct minix_reloc *pmrel, const Elf32_Rel *prel,
			  Elf_Scn *place_scn, uint16_t offset_in_scn)
{
	...
	if (addend * 0x10 == text_sh->sh_addr)
		...
		if (place_scn == text)
			...
			place_scn_name = ".data";
		error("R_386_SEGRELATIVE relocation at place %s:%#" PRIx16
		      " has bad "
...
static void convert_relocs(void)
{
	uint32_t tridx = 0, ftridx = text_n_rels,
		 dridx = text_n_rels + ftext_n_rels;
	Elf_Data *stuff;
	size_t stuff_size;
	const Elf32_Rel *prel;
...
static void output_scn_stuff(Elf_Scn *scn, const Elf32_Shdr *shdr,
			     uint32_t rels_start, uint32_t n_rels,
			     const char *nature)
{
	Elf_Data *stuff;
	size_t stuff_size;
