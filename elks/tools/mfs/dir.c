...
void timefmt(const char *str, u32 usecs)
{
	...
	printf("%s %02d %4d %02d:%02d:%02d ", months[tmb.tm_mon], tmb.tm_mday,
	       tmb.tm_year + 1900, tmb.tm_hour, tmb.tm_min, tmb.tm_sec);
}
...
int dostat_inode(struct minix_fs_dat *fs, int inode)
{
	...
	if (VERSION_2(fs)) {
		...
		if (S_ISCHR(ino->i_mode) || S_ISBLK(ino->i_mode))
			printf("%2d, %3d ", (ino->i_zone[0] >> 8) & 0xff,
			       ino->i_zone[0] & 0xff);
		else
...
{
	int dinode;
	int ninode = make_node(fs, newdir, mode | S_IFDIR, dogetuid(),
			       dogetgid(), 0, NOW, NOW, NOW, &dinode);
