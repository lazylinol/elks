...
static int blocksused(size_t filesize)
{
	...
	while (1) {
		...
		while (1) {
			...
			entry_build_t *child_ent =
				entry_alloc(parent_inode, name);
			if (!child_ent) {
				err = -1;
				break;
			}
			char *child_path = domalloc(
				strlen(parent_path) + 1 + strlen(name) + 1, -1);
			...
			if (!strcmp(name, ".")) {
				...
				if (grand_parent_inode)
					...
					child_ent->inode =
						parent_inode; /* self for root */

			} else {
				inode_build_t *child_inode =
					inode_alloc(parent_inode, child_path);
				if (!child_inode) {
					err = -1;
					break;
				}
...
static int parse_filelist(char *filelist, inode_build_t *parent_inode,
			  char *parent_path)
{
	int err = 0;
	FILE *fp;
...
static int compile_fs(struct minix_fs_dat *fs)
{
	...
	for (; inode_build != (inode_build_t *)&inodes.node;
		...
		if (flags == S_IFDIR) {
			struct stat sb;
			if (*(prefix + inode_build->path) == 0)
				continue; /* skip template dir*/
			if (opt_verbose)
				printf("mkdir %s\n",
				       prefix + inode_build->path);
			if (stat(inode_build->path, &sb))
				die("stat(%s)", inode_build->path);
			av[0] = "mkdir";
			...
			if (opt_verbose)
				printf("mknod %s c %d %d\n",
				       prefix + inode_build->path,
				       inode_build->dev >> MAJOR_SHIFT,
				       inode_build->dev & 0xff);
			av[0] = "mknod";
			av[1] = prefix + inode_build->path;
			av[2] = "c";
			av[3] = major;
			sprintf(major, "%d", inode_build->dev >> MAJOR_SHIFT);
			av[4] = minor;
			sprintf(minor, "%d", inode_build->dev & 0xff);
			av[5] = 0;
			cmd_mknode(fs, 5, av);
			...
			char lnkname[256];
			int cnt = readlink(inode_build->path, lnkname,
					   sizeof(lnkname));
			if (cnt < 0)
				fatalmsg("Can't read link: %s\n",
					 inode_build->path);
			else {
				lnkname[cnt] = 0;
				if (opt_verbose)
...
void cmd_genfs(char *filename, int argc, char **argv)
{
	...
	argv += optind;
	if (argc != 1)
		fatalmsg("Usage: genfs [options] <root_template>");
	...
	if (opt_verbose)
		printf("Generating filesystem from %s\n", dirname);
...
void cmd_addfs(char *filename, int argc, char **argv)
{
	...
	argc--;
	if (argc != 2)
		fatalmsg("Usage: addfs <file_of_filenames> <root_template>");
