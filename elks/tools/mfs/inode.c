...
u32 ino2_zone(struct minix_fs_dat *fs, struct minix2_inode *ino, u32 blk)
{
	...
	if (blk < MINIX2_ZONESZ) {
		...
		if (ino->i_indir_zone == 0)
			return 0;
		dofread(goto_blk(fs->fp, ino->i_indir_zone), iblks,
			sizeof iblks);
		return iblks[blk];
	}
	...
	if (blk < MINIX2_ZONESZ * MINIX2_ZONESZ) {
		...
		if (ino->i_dbl_indr_zone == 0)
			return 0;
		dofread(goto_blk(fs->fp, ino->i_dbl_indr_zone), iblks,
			sizeof iblks);
		if (iblks[blk / MINIX2_ZONESZ] == 0)
			return 0;
		dofread(goto_blk(fs->fp, iblks[blk / MINIX2_ZONESZ]), iblks,
			sizeof iblks);
...
int ino_zone(struct minix_fs_dat *fs, struct minix_inode *ino, int blk)
{
	...
	if (blk < MINIX_ZONESZ * MINIX_ZONESZ) {
		...
		dofread(goto_blk(fs->fp, ino->i_dbl_indr_zone), iblks,
			sizeof iblks);
		if (iblks[blk / MINIX_ZONESZ] == 0)
			return 0;
		dofread(goto_blk(fs->fp, iblks[blk / MINIX_ZONESZ]), iblks,
			sizeof iblks);
 ...
 */
void ino2_setzone(struct minix_fs_dat *fs, struct minix2_inode *ino, u32 blk,
		  u32 zone)
{
	/* Direct block */
	if (blk < 7) {
	...
	if (blk < MINIX2_ZONESZ) {
		...
		if (ino->i_indir_zone == 0) {
			...
			dofread(goto_blk(fs->fp, ino->i_indir_zone), iblks,
				sizeof iblks);
			if (iblks[blk] && iblks[blk] != zone)
				unmark_zone(fs, iblks[blk]);
		}
		iblks[blk] = zone;
		dofwrite(goto_blk(fs->fp, ino->i_indir_zone), iblks,
			 sizeof iblks);
		return;
	}
	/* Double indirect block. */
	...
	if (blk < MINIX2_ZONESZ * MINIX2_ZONESZ) {
		...
		if (iblks[iblk] == 0) {
			...
			dofwrite(goto_blk(fs->fp, ino->i_dbl_indr_zone), iblks,
				 sizeof iblks);
			memset(iblks, 0, sizeof iblks);
		} else {
			dofread(goto_blk(fs->fp, izone = iblks[iblk]), iblks,
				sizeof iblks);
			if (iblks[blk] && iblks[blk] != zone)
				unmark_zone(fs, iblks[blk]);
		}
 ...
 * @param zone - zone to point to
 */
void ino_setzone(struct minix_fs_dat *fs, struct minix_inode *ino, int blk,
		 int zone)
{
	/* Direct block */
	if (blk < 7) {
...
void ino2_freezone(struct minix_fs_dat *fs, struct minix2_inode *ino, u32 blk)
{
	...
	if (blk < MINIX2_ZONESZ) {
		...
		for (i = 0; i < MINIX2_ZONESZ; i++) {
			if (iblks[i]) {
				dofwrite(goto_blk(fs->fp, ino->i_indir_zone),
					 iblks, sizeof iblks);
				return;
			}
		}
	...
	if (blk < MINIX2_ZONESZ * MINIX2_ZONESZ) {
		...
		if (dblks[blk])
			unmark_zone(fs, dblks[blk]);
		dblks[blk] = 0;

		for (i = 0; i < MINIX2_ZONESZ; i++) {
			if (dblks[i]) {
				dofwrite(goto_blk(fs->fp, iblks[iblk]), dblks,
					 sizeof dblks);
				return;
			}
		}
		...
		for (i = 0; i < MINIX2_ZONESZ; i++) {
			if (iblks[i]) {
				dofwrite(goto_blk(fs->fp, ino->i_dbl_indr_zone),
					 iblks, sizeof iblks);
				return;
			}
		}
...
int read_inoblk(struct minix_fs_dat *fs, int inode, u32 blk, u8 *buf)
{
	if (VERSION_2(fs)) {
		...
		if (blk * BLOCK_SIZE > ino->i_size)
			return 0;

		zone = ino2_zone(fs, ino, blk);
		if (zone == 0)
			return 0; /* This is a hole! */
		if (ino->i_size / BLOCK_SIZE == blk)
			bsize = ino->i_size % BLOCK_SIZE;
...
void set_inode(struct minix_fs_dat *fs, int inode, int mode, int nlinks,
	       int uid, int gid, u32 size, u32 atime, u32 mtime, u32 ctime,
	       int clr)
{
