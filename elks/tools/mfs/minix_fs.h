...
#define INODE_BLOCKS(fs)                                            \
	UPPER(INODES(fs), VERSION_2(fs) ? MINIX2_INODES_PER_BLOCK : \
						MINIX_INODES_PER_BLOCK)
#define INODE_BUFFER_SIZE(fs) (INODE_BLOCKS(fs) * BLOCK_SIZE)
#define ZONES(fs)                                            \
	((unsigned long)(VERSION_2(fs) ? (fs)->msb.s_zones : \
					       (fs)->msb.s_nzones))
#define IMAPS(fs) ((fs)->msb.s_imap_blocks)
#define ZMAPS(fs) ((fs)->msb.s_zmap_blocks)
#define FIRSTZONE(fs) ((fs)->msb.s_firstdatazone)
#define NORM_FIRSTZONE(fs) \
	(MINIX_BOOT_BLOCKS + 1 + IMAPS(fs) + ZMAPS(fs) + INODE_BLOCKS(fs))
...
#define get_free_block(fs) \
	(get_free_bit((fs)->zone_bmap, ZMAPS(fs)) + FIRSTZONE(fs) - 1)
