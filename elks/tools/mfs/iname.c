		 ...
		 u32 *blkp, int *offp)
{
	int i, bsz, j;
	u8 blk[BLOCK_SIZE];
	int fdirsize = VERSION_2(fs) ? (INODE2(fs, inode))->i_size :
					     (INODE(fs, inode))->i_size;
...
void dname_add(struct minix_fs_dat *fs, int dinode, const char *name, int inode)
{
	u8 blk[BLOCK_SIZE];
	int dentsz = DIRSIZE(fs);
	int dfsize = VERSION_2(fs) ? INODE2(fs, dinode)->i_size :
					   INODE(fs, dinode)->i_size;
...
void dname_rem(struct minix_fs_dat *fs, int dinode, const char *name)
{
	...
	if (i == -1)
		return;
	i = (VERSION_2(fs) ? INODE2(fs, dinode)->i_size :
				   INODE(fs, dinode)->i_size) -
	    dentsz;
