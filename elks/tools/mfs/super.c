 ...
 */
struct minix_fs_dat *new_fs(const char *fn, int magic, unsigned long fsize,
			    int inodes)
{
	...
	/* Manage inodes */
	if (!inodes)
		inodes = fsize / 3; /* Default number inodes to 1/3 blocks */
   ...
   */
	...
	set_inode(fs, MINIX_ROOT_INO, S_IFDIR | 0755, 2, dogetuid(), dogetgid(),
   ...
   */
	...
	if (!fs->fp)
		die(fn);
	if (fseek(fs->fp, fsize * BLOCK_SIZE - 1, SEEK_SET))
		die("fseek");
	putc(0, fs->fp);
	fflush(fs->fp);
...
void cmd_sysinfo(struct minix_fs_dat *fs)
{
	...
	printf("image 1K blocks: boot %d, super 1, inodemap %d, filemap %d, inode %d, data %ld\n",
	       MINIX_BOOT_BLOCKS, IMAPS(fs), ZMAPS(fs), (int)INODE_BLOCKS(fs),
	       fsize - FIRSTZONE(fs));
	for (i = MINIX_ROOT_INO; i <= INODES(fs); i++)
		if (!inode_in_use(fs, i))
			inodecount++;
	printf("inodes inuse %d, free %d, overhead %d, total %d\n",
	       INODES(fs) - inodecount, inodecount - MINIX_ROOT_INO,
	       MINIX_ROOT_INO, INODES(fs));
	for (i = 1; i < fsize; i++)
		if (!zone_in_use(fs, i))
			blkcount++;
	printf("blocks inuse %ld, free %d, overhead %d, total %ld\n",
	       fsize - FIRSTZONE(fs) - blkcount, blkcount, FIRSTZONE(fs),
	       fsize);
}
...
struct minix_fs_dat *open_fs(const char *fn, int chk)
{
   ...
   */
	...
	dofread(goto_blk(fs->fp, MINIX_SUPER_BLOCK), &(fs->msb),
		sizeof(struct minix_super_block));
	/*
   * Sanity checks ...
   */
	if (FSMAGIC(fs) != MINIX_SUPER_MAGIC &&
	    FSMAGIC(fs) != MINIX_SUPER_MAGIC2 &&
	    FSMAGIC(fs) != MINIX2_SUPER_MAGIC &&
	    FSMAGIC(fs) != MINIX2_SUPER_MAGIC2) {
		fatalmsg("invalid magic fs-type %x", FSMAGIC(fs));
	}
	if (MINIX_VALID_FS != fs->msb.s_state) {
		if (chk)
			fatalmsg("Filesystem %s in an unknown state, use -f",
				 fn);
   ...
   */
	...
	dofread(goto_blk(fs->fp, MINIX_SUPER_BLOCK + 1), fs->inode_bmap,
		IMAPS(fs) * BLOCK_SIZE);
...
{
	goto_blk(fs->fp, MINIX_SUPER_BLOCK);
	dofwrite(goto_blk(fs->fp, MINIX_SUPER_BLOCK), &fs->msb,
		 sizeof(struct minix_super_block));
	dofwrite(goto_blk(fs->fp, MINIX_SUPER_BLOCK + 1), fs->inode_bmap,
		 IMAPS(fs) * BLOCK_SIZE);
	// printf("ZMAPSIZE=%d\n",ZMAPS(fs));
