...
void writefile(struct minix_fs_dat *fs, FILE *fp, int inode)
{
	...
	do {
		...
		if (j != bsz) { /* This is not a hole, so better write it */
			if (bsz < BLOCK_SIZE)
				memset(blk + bsz, 0, BLOCK_SIZE - bsz);
...
void dosymlink(struct minix_fs_dat *fs, char *source, char *lnkpath)
{
	...
	int inode = make_node(fs, lnkpath, 0777 | S_IFLNK, 0, 0, len, NOW, NOW,
			      NOW, NULL);
...
void dohardlink(struct minix_fs_dat *fs, char *source, char *lnkpath)
{
	...
	if (tinode == -1)
		fatalmsg("%s: file doesn't exist", source);

	if (VERSION_2(fs)) {
		struct minix2_inode *ino = INODE2(fs, tinode);
		if (!S_ISREG(ino->i_mode))
			fatalmsg("%s: can only link regular files", source);
		ino->i_nlinks++;
	} else {
	...
	if (find_inode(fs, lnkpath) != -1)
		fatalmsg("%s: already exists", lnkpath);
	if (lname) {
		*lname++ = 0;
	} else {
		dir = ".";
...
void cmd_ln(struct minix_fs_dat *fs, int argc, char **argv)
{
	...
	if (argc != 3)
		fatalmsg("Usage: %s [-s] [source_file] [link_name]\n", argv[0]);
	if (symlink)
		dosymlink(fs, argv[1], argv[2]);
	else
		dohardlink(fs, argv[1], argv[2]);
}
...
void cmd_mknode(struct minix_fs_dat *fs, int argc, char **argv)
{
	...
	if (argc > 6)
		inc = atoi(argv[6]);
	if (argc > 7)
		sscanf(argv[7], "%o", &mode);
	if (inc < 1)
		fatalmsg("Invalid increment value: %d\n", inc);
	if (mode == -1)
		fatalmsg("Invalid mode %s\n", argv[7]);
	...
	if (count > 1 && (type == S_IFBLK || type == S_IFCHR)) {
		...
		do {
			...
			make_node(fs, b, mode | type, uid, gid,
				  DEVNUM(major, minor), NOW, NOW, NOW, NULL);
			minor += inc;
		} while (--count);
	} else {
		make_node(fs, argv[1], mode | type, uid, gid,
			  DEVNUM(major, minor), NOW, NOW, NOW, NULL);
	}
}
...
void cmd_cp(struct minix_fs_dat *fs, int argc, char **argv)
{
	...
	if (argc != 3)
		fatalmsg("Usage: %s [source_file] [dest_dir_or_file]\n",
			 argv[0]);
	if (stat(argv[1], &sb))
		die("stat(%s)", argv[1]);
	...
	if (inode != -1) { /* destination exists*/
		...
		inode = make_node(fs, filename, sb.st_mode,
				  opt_keepuid ? sb.st_uid : 0,
				  opt_keepuid ? sb.st_gid : 0, sb.st_size,
				  sb.st_atime, sb.st_mtime, sb.st_ctime, NULL);
	}
