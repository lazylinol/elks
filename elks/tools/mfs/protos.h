...
void parse_mkfs(int argc, char **argv, int *magic_p, int *nblks_p,
		int *inodes_p);
...
int readfile(struct minix_fs_dat *fs, FILE *fp, const char *path, int type,
	     int ispipe);
...
int ilookup_name(struct minix_fs_dat *fs, int inode, const char *lname,
		 u32 *blkp, int *offp);
void dname_add(struct minix_fs_dat *fs, int dinode, const char *name,
	       int inode);
void dname_rem(struct minix_fs_dat *fs, int dinode, const char *name);
int make_node(struct minix_fs_dat *fs, char *fpath, int mode, int uid, int gid,
	      u32 size, u32 atime, u32 mtime, u32 ctime, int *dinode_p);
