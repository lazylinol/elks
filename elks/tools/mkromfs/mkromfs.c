...
static int parse_dir_dev(inode_build_t *parent_inode, char *child_path)
{
	...
	while (1) {
		...
		while (fgets(buf, sizeof(buf), fp)) {
			if (sscanf(buf, "%s %c %d %d", dev, &type, &major,
				   &minor) != 4)
				break;
			...
			entry_build_t *child_ent =
				entry_alloc(parent_inode, dev + 5);
			if (!child_ent)
				break;

			inode_build_t *child_inode =
				inode_alloc(parent_inode, path);
			if (!child_inode)
				break;

			child_ent->inode = child_inode;

			child_inode->flags = (type == 'c') ? INODE_CHAR :
								   INODE_BLOCK;
		     ...
		     inode_build_t *parent_inode, char *parent_path)
{
	...
	while (1) {
		...
		while (1) {
			...
			char *child_path = malloc(strlen(parent_path) + 1 +
						  strlen(name) + 1);
			...
			if (!strcmp(name, ".")) {
				...
				if (S_ISREG(mode)) {
					...
					if (devfile &&
					    strcmp(&child_path[arglen],
						   "/dev") == 0) {
						err = parse_dir_dev(child_inode,
								    child_path);
						if (err)
							break;
					}
...
static int compile_dir(int fd, inode_build_t *inode)
{
	...
	while (1) {
		...
		entry_build_t *entry =
			(entry_build_t *)inode->entries.node.next;
		while (entry != (entry_build_t *)&inode->entries.node) {
			/* Entry inode index */
...
static int compile_fs()
{
	...
	while (1) {
		...
		off_t offset = sizeof(super_disk_t) +
			       _inodes.count * sizeof(inode_disk_t);
		...
		while (inode_build != (inode_build_t *)&_inodes.node) {
			...
			int count =
				write(fd, &inode_disk, sizeof(inode_disk_t));
