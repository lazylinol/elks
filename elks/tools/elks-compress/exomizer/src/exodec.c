...
static int get_byte(struct dec_ctx *ctx)
{
	int c;
...
static int get_bits(struct dec_ctx *ctx, int count)
{
	int byte_copy = 0;
	int val;
...
static int get_gamma_code(struct dec_ctx *ctx)
{
	int gamma_code;
	/* get bitnum index */
	gamma_code = 0;
...
static int get_cooked_code_phase2(struct dec_ctx *ctx, int index)
{
	int base;
	struct dec_table *tp;
	tp = &ctx->t;
...
static void table_init(struct dec_ctx *ctx, struct dec_table *tp) /* IN/OUT */
{
	int i, end;
	unsigned int a = 0;
	unsigned int b = 0;
		...
		if ((ctx->flags_proto & PFLAG_REUSE_OFFSET) != 0 &&
			...
			LOG(LOG_DEBUG,
			    ("[%d] offset reuse bit = %d, latest = %d\n",
			     buf_size(ctx->outbuf), reuse_offset, offset));
		}
		if (!reuse_offset) {
			i = (len > treshold ? treshold : len) - 1;
			val = ctx->t.table_off[i] +
			      get_bits(ctx, ctx->t.table_bit[i]);
...
literal:
		...
		if (ctx->flags_proto & PFLAG_4_OFFSET_TABLES) {
			LOG(LOG_DEBUG,
			    ("bits read for this iteration %d, total %d.\n",
