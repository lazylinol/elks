...
static int compress(char *infile, char *outfile, int do_text, int do_ftext,
		    int do_data)
{
	int ifd, ofd, efd, n;
	...
	if ((mh.type != MINIX_SPLITID_AHISTORICAL &&
	...
	if (do_text) {
		 ...
		 */
		sprintf(cmd, "%s raw -q -C -b %s,%d,%d -o %s", exomizer_binary,
			infile, szhdr, sztext, exomizer_outfile);
		if (verbose)
			printf("%s\n", cmd);
		...
		unlink(exomizer_outfile);
		if (verbose)
			printf("compressed text from %d to %d\n", sztext,
			       compr_sztext);
	}
	...
	if (do_ftext && szftext) {
		 ...
		 */
		sprintf(cmd, "%s raw -q -C -b %s,%d,%d -o %s", exomizer_binary,
			infile, szhdr + sztext, szftext, exomizer_outfile);
		if (verbose)
			printf("%s\n", cmd);
		unlink(exomizer_outfile);
		if (system(cmd) != 0) {
		...
		unlink(exomizer_outfile);
		if (verbose)
			printf("compressed fartext from %d to %d\n", szftext,
			       compr_szftext);
	}
	...
	if (do_data && szdata) {
		 ...
		 */
		sprintf(cmd, "%s raw -q -C -b %s,%d,%d -o %s", exomizer_binary,
			infile, szhdr + sztext + szftext, szdata,
			exomizer_outfile);
		if (verbose)
			printf("%s\n", cmd);
		unlink(exomizer_outfile);
		...
		unlink(exomizer_outfile);
		if (verbose)
			printf("compressed data from %d to %d\n", szdata,
			       compr_szdata);
	}
...
next:
	...
	if (do_text)
		...
		writesection(ofd, text, sztext, outfile);
	if (do_ftext && szftext)
		writesection(ofd, compr_ftext, compr_szftext, outfile);
	else
		writesection(ofd, ftext, szftext, outfile);
	if (do_data && szdata)
		writesection(ofd, compr_data, compr_szdata, outfile);
	else
		writesection(ofd, data, szdata, outfile);
	...
	printf("%s: compressed from %ld to %ld (%ld%% reduction)\n", infile,
	       orig_size, compr_size, 100 - (compr_size * 100 / orig_size));
...
int main(int ac, char **av)
{
	...
	while (optind < ac) {
		...
		ret = compress(av[optind], outfile ? outfile : outname, do_text,
			       do_ftext, do_data);
		if (ret == 0) {
			if (!keep_infile) {
#if DOHARDLINKS
