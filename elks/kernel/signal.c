...
static void generate(sig_t sig, sigset_t msksig, register struct task_struct *p)
{
	...
	debug_sig("SIGNAL gen_sig %d mask %x action %x:%x pid %d\n", sig,
		  msksig, _FP_SEG(p->sig.handler), _FP_OFF(p->sig.handler),
		  p->pid);
	p->signal |= msksig;
	if ((p->state ==
	     TASK_INTERRUPTIBLE) /* && (p->signal & ~p->blocked) */) {
...
int send_sig(sig_t sig, register struct task_struct *p, int priv)
{
	...
	if (sig != SIGCHLD)
		debug_sig("SIGNAL send_sig %d pid %d\n", sig, p->pid);
	...
	for_each_task(p)
	{
		if (p->pgrp == pgrp) {
			if (p->state < TASK_ZOMBIE)
				...
				debug_sig(
					"SIGNAL skip kill_pg pgrp %d pid %d state %d\n",
					pgrp, p->pid, p->state);
		}
	}
	return err;
...
int kill_process(pid_t pid, sig_t sig, int priv)
{
	...
	for_each_task(p) if (p->pid == pid &&
			     p->state < TASK_ZOMBIE) return send_sig(sig, p, 0);
	return -ESRCH;
}
...
void kill_all(sig_t sig)
{
	...
	for_each_task(p) if (p->state < TASK_ZOMBIE) send_sig(sig, p, 0);
}
...
int sys_kill(pid_t pid, sig_t sig)
{
	...
	count = retval = 0;
	if (pid == (pid_t)-1) {
		for_each_task(p) if (p->pid > 1 && p != pcurrent &&
				     p->state < TASK_ZOMBIE)
		{
			count++;
...
int sys_signal(int signr, __kern_sighandler_t handler)
{
	...
	if (((unsigned int)signr > NSIG) || signr == SIGKILL ||
	    signr == SIGSTOP)
		return -EINVAL;
	if (handler == KERN_SIG_DFL)
		...
		if (_FP_SEG(handler) < current->mm.seg_code->base ||
			...
			debug_sig(
				"SIGNAL sys_signal supplied handler is bogus!\n");
