...
static void del_from_runqueue(register struct task_struct *p)
{
	...
	if (!p->next_run || !p->prev_run)
		panic("SCHED(%d): task not on run-queue, state %d", p->pid,
		      p->state);
	if (p == &idle_task)
...
void schedule(void)
{
	...
	if (intr_count) {
		...
		panic("SCHED: schedule() called from interrupt, intr_count %d",
		      intr_count);
	}
#endif
	...
	if (prev->state == TASK_INTERRUPTIBLE) {
		if (prev->signal ||
		    (prev->timeout && (prev->timeout <= jiffies))) {
			prev->timeout = 0UL;
...
static void update_one_process(struct taks_struct *p, jiff_t ticks, jiff_t user,
			       jiff_t system)
{
