	...
	for_each_task(p)
	{
		if (p->p_parent == current) {
			...
			if (p->state == TASK_ZOMBIE) {
				debug_wait(
					"Zombie orphan pid %d ppid %d removed\n",
					p->pid, p->ppid);
			...
			if (p->state != TASK_UNUSED) {
				debug_wait(
					"Reparenting orphan pid %d ppid %d to init\n",
			...
			if (p->p_parent == current && p->state != TASK_UNUSED) {
				if (p->state == TASK_ZOMBIE ||
					...
					if (pid == (pid_t)-1 || p->pid == pid ||
						...
						p->state =
							TASK_UNUSED; /* unassign task entry*/
						next_task_slot = p;
						task_slots_unused++;
					...
					debug_wait(
						"WAIT(%P) again for pid %d state %d\n",
						p->pid, p->state);
					waitagain = 1;
				}
			}
